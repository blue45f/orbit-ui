    ...
    [
      {
        value: 'production',
        label: 'Production',
        desc: '실제 서비스 사용자에게 배포됩니다. 신중히 선택하세요.',
        badge: 'Live',
        badgeColor: '#10b981',
      },
      {
        value: 'preview',
        label: 'Preview',
        desc: 'PR마다 독립적인 미리보기 URL이 생성됩니다.',
        badge: '권장',
        badgeColor: '#6366f1',
      },
      {
          ...
          {envs.map((item, i) => (
            <div
              ...
              style={{
                ...
                padding: '12px 16px',
                borderBottom:
                  i < envs.length - 1 ? '1px solid var(--sem-eclipse-color-borderSubtle)' : 'none',
                background:
                  env === item.value
...
const RegionPickerDemo = () => {
  ...
  const getLatencyColor = (ms: number) => (ms < 50 ? '#10b981' : ms < 100 ? '#f59e0b' : '#ef4444')
  ...
  return (
    <div style={{ maxWidth: 440 }}>
      ...
      >
        가장 낮은 레이턴시의 지역을 선택하세요. 현재 위치 기준 측정값입니다.
      </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                <span
                  style={{
                    ...
                    height: 6,
                    borderRadius: '50%',
                    background: getLatencyColor(r.latency),
  ...
  const options: { value: string; label: string; desc: string; example: string }[] = [
    {
      value: 'auto',
      label: '자동 감지',
      desc: '프레임워크를 자동으로 인식해 최적 설정을 적용합니다.',
      example: 'Next.js → .next/',
    },
    {
      value: 'static',
      label: '정적 파일',
      desc: '빌드 결과물이 정적 HTML/CSS/JS 파일로 구성됩니다.',
      example: 'Vite → dist/',
    },
    {
      value: 'edge',
      label: 'Edge Function',
      desc: '엣지에서 서버사이드 로직을 실행합니다.',
      example: 'Hono, Remix Edge',
    },
    {
      value: 'serverless',
      label: 'Serverless',
      desc: '서버리스 함수를 통해 동적 렌더링을 처리합니다.',
      example: 'Lambda, DeployPlatform Functions',
    },
  ]
  ...
  return (
    <div style={{ maxWidth: 440, display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div>
        ...
        <div style={{ fontSize: 12, color: 'var(--sem-eclipse-color-foregroundTertiary)' }}>
          M3 Suggestion Chip — 시스템 추천 옵션 패턴
        </div>
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {[
              ...
              <div key={opt.value} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <RadioButtonWithLabel value={opt.value} alignItems="center">
                  <span
                    style={{ fontSize: 13, color: 'var(--sem-eclipse-color-foregroundPrimary)' }}
                  >
                    {opt.label}
                  </span>
  ...
  return (
    <div style={{ maxWidth: 460, display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div>
        ...
        <div style={{ fontSize: 12, color: 'var(--sem-eclipse-color-foregroundTertiary)' }}>
          Primary / Secondary / Tertiary / Surface 역할 기반 색상 시스템
        </div>
      </div>
          ...
          {SCHEMES.map((s) => (
            <div
              ...
              style={{
                ...
                background:
                  scheme === s.value ? s.surface : 'var(--sem-eclipse-color-backgroundPrimary)',
                cursor: 'pointer',
                transition: 'all 0.15s',
              }}
            >
      ...
      <div
        style={{
          ...
          border: `1px solid ${active.primary}30`,
        }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: active.primary }}>
              {active.label} Theme 미리보기
            </div>
            <div style={{ fontSize: 11, color: active.secondary }}>
...
const M3AssistChipDemo = () => {
  ...
  return (
    <div style={{ maxWidth: 460, display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ fontSize: 12, color: 'var(--sem-eclipse-color-foregroundTertiary)' }}>
        M3 Assist Chip — 상태에 따른 권장 액션 제안
      </div>
          ...
          {STATUSES.map((s) => (
            <div
              ...
              style={{
                ...
                background:
                  status === s.value ? s.bg : 'var(--sem-eclipse-color-backgroundPrimary)',
                cursor: 'pointer',
                transition: 'all 0.15s',
              }}
            >
              ...
              <span
                style={{
                  ...
                  color:
                    status === s.value ? s.color : 'var(--sem-eclipse-color-foregroundSecondary)',
                }}
              >
                {s.label}
              </span>
          ...
          {roles.map((r) => (
            <div
              ...
              style={{
                ...
                background:
                  role === r.value ? `${r.color}08` : 'var(--sem-eclipse-color-backgroundPrimary)',
                cursor: 'pointer',
                transition: 'border-color 0.15s, background 0.15s',
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                ...
                <span
                  style={{
                    ...
                    fontWeight: 700,
                    color:
                      role === r.value ? r.color : 'var(--sem-eclipse-color-foregroundPrimary)',
                  }}
                >
                  {r.label}
                </span>
    ...
    monthly: [
      {
        ...
        label: 'Basic',
        price: 9000,
        features: ['컴포넌트 20개', '스토리 100개', '이메일 지원'],
      },
      {
        value: 'pro',
        label: 'Pro',
      ...
      },
      {
        ...
        label: 'Team',
        price: 79000,
        features: ['Pro 모든 기능', '팀 5명', '공유 워크스페이스', 'Slack 지원'],
      },
    ],
    yearly: [
      {
          ...
          {(['monthly', 'yearly'] as const).map((b) => (
            <button
              ...
              style={{
                ...
                fontWeight: 600,
                background:
                  billing === b ? '#6366f1' : 'var(--sem-eclipse-color-backgroundPrimary)',
                color: billing === b ? '#fff' : 'var(--sem-eclipse-color-foregroundTertiary)',
                border: 'none',
                cursor: 'pointer',
            ...
            >
              {b === 'monthly' ? '월간' : '연간'}
              {b === 'yearly' && (
                <span style={{ fontSize: 10, marginLeft: 4, opacity: 0.85 }}>-20%</span>
              )}
            </button>
          ))}
        </div>
          ...
          {current.map((p) => (
            <div
              ...
              style={{
                ...
                padding: '14px 12px',
                borderRadius: 8,
                border: `2px solid ${plan === p.value ? '#6366f1' : 'var(--sem-eclipse-color-borderSubtle)'}`,
                background:
                  plan === p.value ? '#6366f108' : 'var(--sem-eclipse-color-backgroundPrimary)',
                cursor: 'pointer',
              ...
              <div style={{ marginTop: 10, paddingLeft: 4 }}>
                <div
                  style={{
                    ...
                    fontWeight: 700,
                    color:
                      plan === p.value ? '#6366f1' : 'var(--sem-eclipse-color-foregroundPrimary)',
                  }}
                >
                  {p.label}
                </div>
...
const NOTIFY_CHANNELS = [
  {
    value: 'push',
    label: '푸시 알림',
    desc: '기기 잠금 화면 및 알림 센터에 즉시 표시',
    icon: '🔔',
  },
  {
    value: 'email',
    label: '이메일',
    desc: '등록된 이메일 주소로 발송 (최대 5분 지연)',
    icon: '✉️',
  },
  ...
  return (
    <div style={{ maxWidth: 400, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {NOTIFY_CHANNELS.map((c) => (
            <label
              ...
              style={{
                ...
                padding: '12px 14px',
                borderRadius: 10,
                border: `1.5px solid ${channel === c.value ? '#6366f1' : 'var(--sem-eclipse-color-borderSubtle)'}`,
                background:
                  channel === c.value ? '#6366f108' : 'var(--sem-eclipse-color-backgroundPrimary)',
                cursor: 'pointer',
                transition: 'border-color 0.15s',
              }}
            >
      ...
      <div style={{ fontSize: 11, color: 'var(--sem-eclipse-color-foregroundDisabled)' }}>
        PrimitiveUI Radio 패턴 — 아이콘 + 설명 텍스트 선택지
      </div>
    </div>
  )
}
...
export const Primitive_알림_채널_설정: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'PrimitiveUI RadioGroup에서 영감을 받은 알림 채널 선택 UI. 아이콘 + 레이블 + 설명 텍스트 패턴, 선택 시 테두리 강조, 긴급 알림 필터 체크박스 연동.',
      },
    },
  },
...
const DEPLOY_TARGETS: {
  ...
  },
  {
    ...
    badge: 'PR',
    badgeColor: '#6366f1',
    desc: 'PR 브랜치별로 자동 생성되는 미리보기 환경입니다.',
    branch: 'feat/*',
  },
  {
    value: 'development',
    ...
    badge: 'local',
    badgeColor: '#f59e0b',
    desc: '로컬 개발 서버 연동 환경입니다. 외부 노출 없음.',
    branch: 'localhost',
  },
]
    ...
    <div style={{ maxWidth: 420, display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <RadioGroup
        ...
        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
          setTarget(e.target.value as DeployTarget)
        }
        name="deploy-target"
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {DEPLOY_TARGETS.map((t) => (
            <div
              ...
              style={{
                padding: '12px 16px',
                borderRadius: 10,
                border: `1.5px solid ${target === t.value ? t.badgeColor : 'var(--sem-eclipse-color-borderSubtle)'}`,
                background:
                  target === t.value
                    ? `${t.badgeColor}08`
                    : 'var(--sem-eclipse-color-backgroundPrimary)',
            ...
            >
              ...
              {target === t.value && (
                ...
                >
                  <div
                    style={{ fontSize: 11, color: 'var(--sem-eclipse-color-foregroundSecondary)' }}
                  >
                    {t.desc}
                  </div>
                  <div style={{ fontSize: 11, fontFamily: 'monospace', color: t.badgeColor }}>
                    branch: {t.branch}
                  </div>
                </div>
              )}
      ...
      >
        선택됨:{' '}
        <span style={{ fontWeight: 700, color: selected.badgeColor }}>{selected.label}</span> —{' '}
        {selected.desc}
      </div>
      <div style={{ fontSize: 11, color: 'var(--sem-eclipse-color-foregroundDisabled)' }}>
        DeployPlatform Design 배포 환경 선택 패턴 — 컬러 코딩 + 상태 확장
      </div>
    </div>
  )
}
...
export const DeployPlatform_배포_환경_선택_Cycle112: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'DeployPlatform 대시보드 배포 환경 선택 UI. Production/Preview/Development를 RadioGroup으로 선택하고, 선택 시 브랜치 패턴과 설명이 확장되어 노출됩니다. 환경별 컬러 코딩 패턴.',
      },
    },
  },
...
const AUTH_METHODS: {
...
}[] = [
  {
    ...
    level: 1,
    color: '#ef4444',
    hint: '기본 보안. 강력한 비밀번호 정책 별도 설정 권장.',
  },
  {
    value: 'sso',
    label: 'SSO (OAuth 2.0)',
    level: 2,
    color: '#f59e0b',
    hint: 'Reference / CodeHost 계정으로 로그인. 팀 계정 관리 편리.',
  },
  {
    value: 'mfa',
    label: 'MFA + SSO',
    level: 3,
    color: '#10b981',
    hint: '최고 보안. OTP 앱 또는 하드웨어 키 2단계 인증 필수.',
  },
]
  ...
  return (
    <div style={{ maxWidth: 380, display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div>
        ...
        <div style={{ fontSize: 12, color: 'var(--sem-eclipse-color-foregroundTertiary)' }}>
          보안 수준이 높을수록 계정 탈취 위험이 낮아집니다
        </div>
      </div>
      <RadioGroup
        value={method}
        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
          setMethod(e.target.value as AuthMethod)
        }
        name="auth-method"
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {AUTH_METHODS.map((m) => (
            <div
              ...
              style={{
                padding: '12px 14px',
                borderRadius: 10,
                border: `1.5px solid ${method === m.value ? m.color : 'var(--sem-eclipse-color-borderSubtle)'}`,
                cursor: 'pointer',
                transition: 'all 0.15s',
              }}
            >
                  ...
                  {[1, 2, 3].map((i) => (
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: 2,
                        background:
                          i <= m.level ? m.color : 'var(--sem-eclipse-color-borderSubtle)',
      ...
      >
        <span style={{ fontWeight: 700, color: selected.color }}>보안 레벨 {selected.level}/3</span>{' '}
        — {selected.hint}
      </div>
      <div style={{ fontSize: 11, color: 'var(--sem-eclipse-color-foregroundDisabled)' }}>
        PrimitiveUI + DeployPlatform — 보안 레벨 시각화 선택지 패턴
      </div>
    </div>
  )
}
...
export const Primitive_DeployPlatform_보안_인증_방식: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'PrimitiveUI RadioGroup + DeployPlatform 보안 설정 UI 조합. 인증 방식별 보안 레벨을 3단계 블록으로 시각화하고, 선택 시 힌트 텍스트가 하단에 표시됩니다.',
      },
    },
  },
    ...
    <div style={{ width: 360, fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>
          EnterpriseUI Radio 수직 목록 + 설명 패턴
        </div>
      </div>
      <RadioGroup
        value={freq}
        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFreq(e.target.value)}
        name="notif-freq"
      >
              ...
              {f.badge && (
                <span
                  style={{
                    ...
                    padding: '2px 6px',
                    borderRadius: 8,
                    background: f.value === 'realtime' ? '#dbeafe' : '#d1fae5',
                    color: f.value === 'realtime' ? '#1d4ed8' : '#065f46',
                  }}
                >
                  {f.badge}
                </span>
  ...
  return (
    <div style={{ width: 380, fontFamily: 'system-ui, sans-serif' }}>
      ...
      >
        <span>
          선택:{' '}
          <strong>{TAILWIND_PAYMENT_METHODS_188.find((m) => m.value === method)?.label}</strong>
        </span>
        <button
          style={{
            padding: '5px 14px',
...
const EnterpriseUI_DEPLOY_STRATEGIES_188 = [
  {
    ...
    desc: '점진적 교체. 무중단 배포. 평균 3분',
    risk: 'low',
  },
  {
    value: 'bluegreen',
    label: 'Blue/Green',
    desc: '전환 즉시 완료. 롤백 쉬움. 비용 2배',
    risk: 'low',
  },
  ...
  return (
    <div style={{ width: 400, fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>
          EnterpriseUI + UtilityUI 복합 선택 패턴
        </div>
      </div>
      <RadioGroup
        value={strategy}
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                    <span
                      style={{
                        fontSize: 13,
                        fontWeight: strategy === s.value ? 700 : 500,
                        color: '#0f172a',
                      }}
                    >
                      {s.label}
