...
const SignupFormDemo = () => {
  ...
  const errors = {
    ...
    phone:
      touched.phone && !/^[0-9-]{10,13}$/.test(form.phone.replace(/\s/g, ''))
        ? '올바른 전화번호 형식이 아닙니다'
        : '',
    company: '',
  }
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          ...
          <FormFieldWrapper label="회사명" helperText="선택 사항" errorText="" isError={false}>
            <TextField
      ...
      <button
        ...
        style={{
          ...
          background: isValid ? 'linear-gradient(135deg, #6366f1, #8b5cf6)' : '#e2e8f0',
...
export const EnterpriseUI_Variant_비교: Story = {
  ...
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px', width: '560px' }}>
      ...
      >
        <div
          style={{ fontSize: '12px', fontWeight: '600', color: '#64748b', marginBottom: '10px' }}
        >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {[
            {
              enterpriseUi: 'outlined',
              orbit: 'TextField (기본)',
              note: '테두리 있는 형태 - 동일',
            },
            {
              enterpriseUi: 'filled',
              orbit: 'theme prop으로 배경색 적용',
              note: '배경 채워진 형태',
            },
            {
              enterpriseUi: 'standard',
              orbit: 'theme prop으로 언더라인만',
              note: '언더라인만 있는 형태',
            },
          ].map((row) => (
            <div
              key={row.enterpriseUi}
              style={{
                display: 'grid',
        ...
        >
          {filtered.map((item, i) => (
            <div
              ...
              onMouseEnter={(e) => {
                ;(e.currentTarget as HTMLDivElement).style.background = '#f2f2f7'
              }}
              onMouseLeave={(e) => {
                ;(e.currentTarget as HTMLDivElement).style.background = '#fff'
              }}
            >
              <span style={{ fontSize: 12, color: '#6366f1', width: 20, textAlign: 'center' }}>
                C
              </span>
              <span>{item}</span>
            </div>
    ...
    >
      ...
      <div style={{ padding: '10px 20px', background: '#fafafa', borderTop: '1px solid #f1f5f9' }}>
        <span style={{ fontSize: 11, color: '#94a3b8' }}>
          WorkspaceEditor — 빠른 캡처 받은 편지함 패턴
        </span>
      </div>
    </div>
  )
...
const validateRules = (value: string) =>
  ({
    ...
    alphaNum:
      value.length > 0 ? (/^[a-zA-Z0-9가-힣_-]+$/.test(value) ? 'valid' : 'invalid') : 'idle',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <label
          style={{
            fontSize: 13,
            fontWeight: 600,
            color: activeState === 'error' ? '#dc2626' : '#374151',
          }}
        >
          이메일
        </label>
...
const PasswordStrengthDemo = () => {
  ...
  return (
    <div style={{ maxWidth: 360, display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <label htmlFor="pwd-field" style={{ fontSize: 13, fontWeight: 600, color: '#0f172a' }}>
          비밀번호
        </label>
        ...
        <>
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span id="pwd-strength-desc" style={{ fontSize: 12, color: '#64748b' }}>
                강도
              </span>
    ...
    return (
      <div style={{ maxWidth: 360, textAlign: 'center', padding: '32px' }}>
        ...
        <button
          onClick={() => {
            setSubmitted(false)
            setValues({ username: '', email: '', phone: '' })
            setTouched({})
          }}
          style={{
            marginTop: 16,
...
function ComposableUIOTPRender() {
  ...
  return (
    ...
    >
          ...
          <div key={i}>
            ...
            <input
              ...
              style={{
                ...
                background:
                  verified === 'error' ? '#fff7f7' : verified === 'success' ? '#f0fdf4' : '#fff',
                textAlign: 'center',
                fontSize: 20,
                fontWeight: 700,
                color: '#0f172a',
      ...
      <div style={{ fontSize: 11, color: '#94a3b8' }}>
        ComposableUI InputOTP 패턴 — 자동 포커스 이동 + Backspace 핸들링
      </div>
    </div>
  )
}
...
const INLINE_FIELDS = [
  ...
  { id: 'slug', label: 'URL 슬러그', value: 'orbit-ui-ds', multiline: false },
  {
    id: 'desc',
    label: '설명',
    value: 'React 기반 DesignTool-first 디자인 시스템',
    multiline: false,
  },
]
...
export const Accessible_태그_입력_필드: Story = {
  ...
  render: function AccessibleTagInputRender() {
    ...
    function addTag(raw: string) {
      ...
      if (tag.length > 20) {
        setError('태그는 20자 이하여야 합니다')
        return
      }
      if (tags.length >= 10) {
        setError('최대 10개 태그까지 추가할 수 있습니다')
        return
      }
    ...
    return (
      <div style={{ width: 400, fontFamily: 'system-ui, sans-serif' }}>
        ...
        <div style={{ marginTop: 4, fontSize: 11, color: '#94a3b8' }}>
          {tags.length}/10 태그 · Enter 또는 쉼표로 추가 · Backspace로 마지막 태그 제거
        </div>
      </div>
    )
  },
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                {scopes.map((s) => (
                  <div
                    ...
                    style={{
                      ...
                      padding: '10px 12px',
                      borderRadius: 8,
                      border: `1.5px solid ${s.on ? '#6366f1' : '#e2e8f0'}`,
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: 16,
                        borderRadius: 4,
                        border: `2px solid ${s.on ? '#6366f1' : '#cbd5e1'}`,
                        background: s.on ? '#6366f1' : '#fff',
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        flexShrink: 0,
                ...
                {['7d', '30d', '90d', '무제한'].map((opt) => (
                  <button
                    ...
                    style={{
                      padding: '6px 12px',
                      borderRadius: 7,
                      border: `1.5px solid ${expiry === opt ? '#6366f1' : '#e2e8f0'}`,
                      background: expiry === opt ? '#eef2ff' : '#fff',
                      color: expiry === opt ? '#4338ca' : '#475569',
                      fontSize: 12,
                      fontWeight: 500,
                      cursor: 'pointer',
                    }}
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 8,
                border: 'none',
                background: canGenerate ? '#0f172a' : '#f1f5f9',
      ...
      <div style={{ width: 360, fontFamily: 'system-ui, sans-serif' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            ...
            />
            {nameError && (
              <div style={{ fontSize: 11, color: '#ef4444', marginTop: 4 }}>{nameError}</div>
            )}
          </div>
          <div>
            <label
            ...
            />
            {emailError && (
              <div style={{ fontSize: 11, color: '#ef4444', marginTop: 4 }}>{emailError}</div>
            )}
          </div>
          <div>
            <label
                  ...
                  {[1, 2, 3, 4].map((level) => (
                    <div
                      ...
                      style={{
                        ...
                        height: 3,
                        borderRadius: 2,
                        background: level <= pwStrength ? strengthColor : '#e5e7eb',
              ...
              {priorities.map((p) => (
                <button
                  ...
                  style={{
                    padding: '4px 10px',
                    borderRadius: 6,
                    border: `1.5px solid ${priority === p.value ? p.color : '#e5e7eb'}`,
          ...
          {FILTERS.map((f) => (
            <button
              ...
              style={{
                padding: '4px 10px',
                borderRadius: 20,
                border: `1.5px solid ${activeFilters.includes(f) ? '#3b82f6' : '#e5e7eb'}`,
