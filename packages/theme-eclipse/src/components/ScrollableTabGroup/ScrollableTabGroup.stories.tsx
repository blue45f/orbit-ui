...
const categories = [
  {
    ...
    icon: '🏠',
    count: null,
    desc: 'Browse all available items across every category.',
  },
  {
    id: 'featured',
    label: 'Featured',
    icon: '⭐',
    count: null,
    desc: 'Hand-picked featured items curated by our team.',
  },
  {
    id: 'new',
    label: 'New Arrivals',
    icon: '✨',
    count: 12,
    desc: 'Freshly added items in the last 7 days.',
  },
  {
    id: 'design',
    label: 'Design',
    icon: '🎨',
    count: null,
    desc: 'UI kits, templates, and design resources.',
  },
  {
    id: 'dev',
    label: 'Development',
    icon: '💻',
    count: null,
    desc: 'Code libraries, APIs, and developer tools.',
  },
  {
    id: 'motion',
    label: 'Motion',
    icon: '🎬',
    count: null,
    desc: 'Animation presets and motion design files.',
  },
  {
    id: 'icons',
    label: 'Icons',
    icon: '🔷',
    count: null,
    desc: 'Icon packs in SVG, PNG, and Lottie format.',
  },
  {
    id: 'fonts',
    label: 'Typography',
    icon: '✍️',
    count: null,
    desc: 'Premium fonts and type system configurations.',
  },
  {
    id: 'color',
    label: 'Color',
    icon: '🎭',
    count: null,
    desc: 'Color palettes, gradients, and token systems.',
  },
  {
    id: 'promo',
    label: 'On Sale',
    icon: '🔥',
    count: 5,
    desc: "Limited-time deals — grab them before they're gone!",
  },
]
      ...
      <div style={{ padding: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
          ...
          <div>
            ...
            >
              {current?.count !== null ? `${current?.count} items` : 'All items'}
            </div>
          </div>
        </div>
        <div
...
const DeployPlatformDeploymentTabsRender = () => {
  ...
  const filtered =
    activeEnv.id === 'all' ? deployments : deployments.filter((d) => d.env === activeEnv.id)
          ...
          >
            <span
              style={{
                ...
                height: '8px',
                borderRadius: '50%',
                background: statusColors[dep.status] ?? '#94a3b8',
                flexShrink: 0,
              }}
            />
            ...
            <span
              style={{ fontSize: '11px', color: '#94a3b8', minWidth: '48px', textAlign: 'right' }}
            >
              {dep.time}
            </span>
          </div>
...
const EcommerceTabFilterRender = () => {
  ...
  const filtered =
    activeId === 'all' ? productItems : productItems.filter((p) => p.category === activeId)
    ...
    <div style={{ width: '380px', fontFamily: '"Segoe UI", sans-serif' }}>
      ...
      >
        ...
        <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {filtered.length === 0 && (
            <div
              style={{ padding: '32px', textAlign: 'center', color: '#94a3b8', fontSize: '13px' }}
            >
              해당 카테고리의 상품이 없습니다.
            </div>
          )}
...
const messages = [
  {
    ...
    folder: 'inbox',
    sender: 'Orbit UI 팀',
    subject: '[공지] v2.0 업데이트 안내',
    preview: '안녕하세요! Orbit UI v2.0이 출시되었습니다.',
    time: '오전 10:32',
    unread: true,
  },
  {
    ...
    folder: 'inbox',
    sender: 'CodeHost',
    subject: 'New pull request: feat/dark-mode',
    preview: 'hjunkim opened a pull request in orbit-ui/orbit-ui...',
    time: '오전 9:15',
    unread: true,
  },
  {
    ...
    folder: 'inbox',
    sender: '디자인팀',
    subject: '피그마 파일 업데이트 완료',
    preview: '버튼 컴포넌트 토큰 재정의가 완료되었습니다.',
    time: '어제',
    unread: false,
  },
  {
              ...
              <ScrollableTabGroup.TabTrailing>
                <span
                  style={{
                    ...
                    fontSize: 10,
                    fontWeight: 700,
                    background: statusColors[item.label]
                      ? `${statusColors[item.label]}18`
                      : '#f1f5f9',
...
const TAILWIND_SECTIONS = [
  {
    ...
    label: '개요',
    badge: null,
    summary: '프로젝트 전체 현황과 최근 활동을 확인합니다.',
    items: ['전체 태스크 48개', '완료 31개', '진행 중 12개', '지연 5개'],
  },
  {
    id: 'tasks',
    label: '태스크',
    badge: 17,
    summary: '할당된 태스크 목록과 우선순위를 관리합니다.',
    items: ['긴급 3개', '높음 6개', '보통 5개', '낮음 3개'],
  },
  {
    id: 'members',
    label: '팀원',
    badge: null,
    summary: '프로젝트에 참여 중인 팀원 현황을 확인합니다.',
    items: ['개발자 4명', '디자이너 2명', 'PM 1명', 'QA 2명'],
  },
  {
    id: 'milestones',
    label: '마일스톤',
    badge: 2,
    summary: '주요 마일스톤과 달성률을 추적합니다.',
    items: ['베타 배포 D-12', '정식 출시 D-34', '리트로 D-8', '플래닝 D-2'],
  },
  {
    id: 'docs',
    label: '문서',
    badge: null,
    summary: '프로젝트 관련 문서와 가이드를 관리합니다.',
    items: ['API 문서', '디자인 가이드', 'PRD', '온보딩'],
  },
  {
    id: 'settings',
    label: '설정',
    badge: null,
    summary: '프로젝트 기본 정보와 권한을 설정합니다.',
    items: ['알림 설정', '멤버 권한', '연동 서비스', '보안'],
  },
]
...
export const UtilityCSS_프로젝트_내비게이션_탭: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'UtilityUI의 수평 탭 내비게이션 패턴. 배지 카운터가 달린 섹션 탭으로 프로젝트 대시보드 내부를 구조화합니다. 활성 탭 콘텐츠가 아래 패널에 즉시 반영됩니다.',
      },
    },
  },
...
export const EnterpriseUI_기간별_지표_대시보드: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'EnterpriseUI Tabs의 scrollButtons="auto" 패턴. 1일~1년 기간 탭을 선택하면 매출/사용자/주문/환불 지표가 비례 계산되어 즉시 업데이트됩니다. 증감률을 색상으로 직관적으로 표현합니다.',
      },
    },
  },
...
const NATIVE_APPS = [
  {
    ...
    label: '메일',
    count: 14,
    content: ['받은 편지함 14개 미읽음', '보낸 편지함', '스팸 3개', '임시보관함 2개'],
  },
  {
    id: 'notes',
    label: '메모',
    count: null,
    content: ['오늘 추가된 메모 3개', '최근 7일 12개', '폴더 5개', '태그된 메모 8개'],
  },
  {
    id: 'calendar',
    label: '캘린더',
    count: 3,
    content: ['오늘 일정 3개', '이번 주 9개', '다음 주 5개', '반복 일정 4개'],
  },
  {
    id: 'reminders',
    label: '알림',
    count: 7,
    content: ['오늘 마감 7개', '예정 12개', '완료 34개', '플래그 2개'],
  },
  {
    id: 'photos',
    label: '사진',
    count: null,
    content: ['최근 48장', '앨범 12개', '공유 앨범 3개', '즐겨찾기 67장'],
  },
  {
    id: 'files',
    label: '파일',
    count: null,
    content: ['최근 파일 8개', '공유된 파일 4개', '태그 6개', '용량 4.2GB'],
  },
]
...
export const Platform_HIG_앱_섹션_탭: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Platform HIG의 iOS 세그먼티드/탭 패턴을 ScrollableTabGroup으로 구현. 시스템 앱 스타일의 그룹화된 콘텐츠 탭과 SF 심볼 스타일 배지를 사용해 알림 수를 빨간색으로 강조합니다.',
      },
    },
  },
...
const CYCLE_CONTENT: Record<
  IssueTrackerCycleStatus,
  { title: string; dates: string; issues: CycleIssue[] }
> = {
  active: {
              ...
              >
                <div
                  style={{
                    ...
                    height: '100%',
                    borderRadius: 2,
                    background: issue.progress === 100 ? '#10b981' : '#6366f1',
...
const TRACKER_TEAM_TABS: Array<{
  id: IssueTrackerTeamId
  label: string
  color: string
  count: number
}> = [
...
type IssueTrackerIssue94 = {
  id: string
  title: string
  team: IssueTrackerTeamId
...
function IssueTrackerTeamFilterTabsRender() {
  ...
  const filtered =
    activeId === 'all' ? TRACKER_ISSUES : TRACKER_ISSUES.filter((i) => i.team === activeId)
...
const TRACKER_QUARTER_TABS: Array<{
  id: IssueTrackerQuarter
  label: string
  year: number
...
export const Ant_바코드_뱃지_탭: Story = {
  ...
  render: function Render() {
    ...
    const filtered =
      selectedIndex === 0 ? items : items.filter((it) => it.status === selectedIndex - 1)
...
export const EnterpriseUI_동적_탭_추가_삭제: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'EnterpriseUI 동적 탭(closable tabs) 패턴. 탭 우측 × 버튼으로 탭을 삭제하고 + 버튼으로 새 탭을 추가. 각 탭에 항목 카운트 뱃지 포함. 데이터 필터링 워크스페이스에서 활용.',
      },
    },
  },
...
function DataProductWidgetMetricTabRender() {
  ...
  const metrics = [
    {
      label: '방문자',
      data: [1200, 1850, 1430, 2100, 1920, 2350, 2180],
      unit: '명',
      color: '#6366f1',
      total: 13030,
    },
    {
      label: '전환율',
      data: [2.1, 3.4, 2.8, 4.2, 3.9, 4.8, 4.1],
      unit: '%',
      color: '#10b981',
      total: 3.6,
    },
    {
      label: '매출',
      data: [420, 630, 510, 780, 690, 890, 820],
      unit: '만원',
      color: '#f59e0b',
      total: 4740,
    },
    ...
    <div style={{ maxWidth: 500, fontFamily: 'system-ui, sans-serif' }}>
      ...
      >
        ...
        >
          ...
          <span style={{ fontSize: 22, fontWeight: 800, color: active.color }}>
            {active.total.toLocaleString()}
            <span style={{ fontSize: 12, marginLeft: 2 }}>{active.unit}</span>
          </span>
        </div>
            ...
            >
              <div
                style={{
                  ...
                  opacity: 0.6 + (idx / active.data.length) * 0.4,
                  height: `${(val / max) * 72}px`,
                  transition: 'height 0.3s ease',
                }}
              />
              <span style={{ fontSize: 8, color: 'var(--sem-eclipse-color-foregroundDisabled)' }}>
                {['월', '화', '수', '목', '금', '토', '일'][idx]}
              </span>
            </div>
          ))}
...
export const DataProduct_대시보드_위젯_탭: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'DataProductUI 데이터 시각화 탭 패턴. 방문자/전환율/매출/이탈률 지표를 탭으로 전환하면 차트 데이터와 총합 수치가 실시간으로 업데이트. 대시보드 위젯에서 활용.',
      },
    },
  },
    ...
    <div style={{ maxWidth: 560, fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div
        style={{
          ...
          padding: '16px',
          borderRadius: 10,
          border: `1px solid ${statusColor[active.status]}40`,
          background: 'var(--sem-eclipse-color-surfaceDefault)',
        }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
          ...
          <span
            style={{
              ...
              padding: '1px 7px',
              borderRadius: 10,
              background: `${statusColor[active.status]}20`,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          {[
            { label: 'CPU', val: active.cpu, unit: '%' },
            { label: 'Memory', val: active.mem, unit: '%' },
            { label: 'Req/min', val: active.req, unit: '' },
          ].map((m) => (
            <div
              key={m.label}
              style={{
...
export const EnterpriseUI_DataProduct_서버_모니터링_탭: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'EnterpriseUI 탭 상태 표시 + DataProduct 데이터 시각화 조합. 각 서버 탭에 healthy/warning/critical 상태 도트 표시, 선택 시 CPU/Memory/요청수를 지표 카드로 표시. DevOps 모니터링 대시보드 패턴.',
      },
    },
  },
              ...
              <ScrollableTabGroup.TabTrailing>
                <span
                  style={{
                    ...
                    padding: '0px 5px',
                    borderRadius: 99,
                    background: active === logTabs.indexOf(tab) ? current.color + '20' : '#f1f5f9',
                    color: active === logTabs.indexOf(tab) ? current.color : '#94a3b8',
                  }}
                >
                  {tab.count}
                </span>
        ...
        {logEntries.map((entry, i) => (
          <div
            ...
            style={{
              ...
              gap: 10,
              padding: '8px 0',
              borderBottom: i < logEntries.length - 1 ? '1px solid #f8fafc' : 'none',
            }}
          >
            <div
              style={{
                ...
                height: 6,
                borderRadius: '50%',
                background: severityColor[entry.severity as keyof typeof severityColor],
                flexShrink: 0,
              }}
            />
            <span
...
export const AppUI_감사_로그_탭: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'AppUI Tabs의 우측 count indicator 패턴 적용. 각 로그 카테고리 탭에 활성 상태에 따라 색상이 변하는 count 배지 표시. 보안/접근/변경/시스템/API 분류로 감사 로그를 탐색하는 엔터프라이즈 패턴.',
      },
    },
  },
...
function AntIssueStatusTabRender() {
  ...
  const issues = [
    {
      id: 'ORB-241',
      title: '다크모드 전환 시 Slider 트랙 색상 깜빡임',
      priority: 'high',
      assignee: 'K',
      label: 'bug',
    },
    {
      id: 'ORB-238',
      title: 'Calendar disabled 날짜 키보드 접근성 개선',
      priority: 'medium',
      assignee: 'J',
      label: 'a11y',
    },
    {
      id: 'ORB-235',
      title: 'DataTable 헤더 고정 시 그림자 미적용 이슈',
      priority: 'medium',
      assignee: 'H',
      label: 'bug',
    },
    {
      id: 'ORB-232',
      title: 'SegmentedControl RTL 레이아웃 지원',
      priority: 'low',
      assignee: 'M',
      label: 'enhancement',
    },
        ...
        <ScrollableTabGroup selectedIndex={active} onTabChange={setActive}>
          {statusTabs.map((tab, i) => (
            <ScrollableTabGroup.Tab key={tab.id} value={tab.id}>
              <ScrollableTabGroup.TabLeading>
                <div
                  style={{ width: 6, height: 6, borderRadius: '50%', background: tab.statusColor }}
                />
              </ScrollableTabGroup.TabLeading>
              <ScrollableTabGroup.TabCenter>{tab.label}</ScrollableTabGroup.TabCenter>
              <ScrollableTabGroup.TabTrailing>
                <span
                  style={{
                    fontSize: 9,
                    color: active === i ? tab.statusColor : '#94a3b8',
                    fontWeight: 700,
                  }}
                >
                  {tab.count}
          ...
          >
            <div
              style={{
                ...
                height: 8,
                borderRadius: '50%',
                background: priorityColor[issue.priority as keyof typeof priorityColor],
                flexShrink: 0,
              }}
            />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                <span
                  style={{ fontSize: 9, color: '#94a3b8', fontFamily: 'monospace', flexShrink: 0 }}
                >
                  {issue.id}
                </span>
                <span
              ...
              <span
                style={{
                  ...
                  padding: '1px 6px',
                  borderRadius: 99,
                  background: labelColor[issue.label]?.bg,
                  color: labelColor[issue.label]?.text,
                  fontWeight: 600,
                }}
              >
                {issue.label}
...
export const Ant_이슈_상태_탭: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Ant Design Tabs의 상태 인디케이터 패턴 적용. 각 탭 앞에 상태 색상 도트를 배치하고 count를 탭 뒤에 표시. 이슈 트래커의 Open/InProgress/Review/Blocked/Done/Closed 6단계 상태를 탭으로 구현.',
      },
    },
  },
      ...
      <div style={{ padding: '14px 16px 0', borderBottom: '1px solid #f1f5f9' }}>
        ...
        <ScrollableTabGroup selectedIndex={active} onTabChange={setActive}>
          {analyticsTabs.map((tab) => (
            <ScrollableTabGroup.Tab key={tab.id} value={tab.id}>
              <ScrollableTabGroup.TabLeading>
                <span style={{ fontSize: 10, color: tab.live ? '#ef4444' : '#94a3b8' }}>
                  {tab.icon}
                </span>
              </ScrollableTabGroup.TabLeading>
              <ScrollableTabGroup.TabCenter>{tab.label}</ScrollableTabGroup.TabCenter>
...
export const AppUI_Ant_애널리틱스_탭: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'AppUI 탭의 아이콘+텍스트 조합 + Ant Design 실시간 인디케이터 패턴. 애널리틱스 대시보드 탭 — 개요/획득/행동/전환/리텐션/실시간(LIVE 배지). 각 탭 전환 시 해당 지표 카드 그리드 표시.',
      },
    },
  },
