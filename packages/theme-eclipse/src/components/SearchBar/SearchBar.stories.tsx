...
const AutocompleteDemoRender = () => {
  ...
  const filtered =
    query.length > 0 ? SUGGESTIONS.filter((s) => s.toLowerCase().includes(query.toLowerCase())) : []
        ...
        >
          {filtered.map((item) => (
            <li
              ...
              onMouseEnter={(e) => {
                ;(e.currentTarget as HTMLLIElement).style.background = '#f8fafc'
              }}
              onMouseLeave={(e) => {
                ;(e.currentTarget as HTMLLIElement).style.background =
                  item === selected ? '#f1f5f9' : 'transparent'
              }}
            >
              {item}
            </li>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', width: '480px' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
        {FILTER_CATEGORIES.map((cat) => (
          <span key={cat} style={{ opacity: activeFilters.includes(cat) ? 1 : 0.5 }}>
            <Chip onClick={() => toggleFilter(cat)}>{cat}</Chip>
          </span>
        ))}
      </div>
      <p style={{ fontSize: '12px', color: '#94a3b8', margin: 0 }}>
        활성 필터: {activeFilters.length > 0 ? activeFilters.join(', ') : '없음'}
        {query ? ` · 검색어: "${query}"` : ''}
      </p>
    </div>
  )
...
function AccessibleUserSearchRender() {
  ...
  const filtered =
    query.length > 0
      ? MOCK_USERS.filter(
          (u) => u.name.includes(query) || u.role.toLowerCase().includes(query.toLowerCase())
        )
      : MOCK_USERS
    ...
    <div style={{ width: '380px' }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        {open && (
          ...
          >
            {filtered.length === 0 && (
              <li style={{ padding: '12px 16px', fontSize: 14, color: '#94a3b8' }}>
                검색 결과 없음
              </li>
            )}
            {filtered.map((user) => (
...
function Material3ScopeSearchRender() {
  ...
  const results =
    query.length > 0
      ? M3_RESULTS[scope].filter((r) => r.title.toLowerCase().includes(query.toLowerCase()))
      : M3_RESULTS[scope]

  return (
    <div style={{ width: '420px' }}>
      <SearchBar value={query} placeholder="검색..." onChange={(e) => setQuery(e.target.value)} />
...
const DataProductGlobalSearchRender = () => {
  ...
  return (
    <div style={{ width: 520, fontFamily: 'system-ui, sans-serif' }}>
      <SearchBar placeholder="프로젝트 전체 검색..." onChange={handleChange} value={query} />
      {query && (
        <div
          style={{
            marginTop: 4,
            ...
            {TABS.map((tab) => (
              <button
                ...
                style={{
                  ...
                  borderBottom:
                    activeTab === tab.id ? '2px solid #0f172a' : '2px solid transparent',
                  marginBottom: -1,
                }}
              >
                {tab.label}
...
const IssueTrackerIssueSearchRender = () => {
  ...
  const ISSUES: {
    ...
    {
      id: 'ORB-138',
      title: 'TextField 접근성 개선 (aria-label)',
      status: 'todo',
      priority: 'high',
      assignee: 'JH',
    },
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((issue, idx) => (
            ...
            >
              ...
              />
              <span
                style={{ fontSize: 10, fontFamily: 'monospace', color: '#94a3b8', flexShrink: 0 }}
              >
                {issue.id}
              </span>
              <span style={{ fontSize: 13, flex: 1, color: '#1e293b', fontWeight: 400 }}>
                {issue.title}
              </span>
              <span
                style={{
    ...
    <div style={{ width: 480 }}>
      ...
      >
        ...
        <div style={{ maxHeight: 320, overflowY: 'auto' }}>
          ...
          {query && !hasResults && (
            <div
              style={{ padding: '24px 14px', textAlign: 'center', fontSize: 13, color: '#94a3b8' }}
            >
...
const ComposableUIDataProductFuzzySearchRender = () => {
  ...
  const scored = FUZZY_ITEMS.map((item) => {
    const lowerLabel = item.label.toLowerCase()
    const lowerQuery = query.toLowerCase()
    let score = 0
    let qi = 0
    for (let i = 0; i < lowerLabel.length && qi < lowerQuery.length; i++) {
      if (lowerLabel[i] === lowerQuery[qi]) {
        score++
        qi++
      }
    }
  ...
  render: function ComposableUIFindInPageRender() {
    ...
    function highlight(line: string, lineIdx: number) {
      ...
      for (;;) {
        ...
        const isCurrent =
          matches[safeIdx]?.lineIdx === lineIdx &&
          matches.findIndex((m) => m.lineIdx === lineIdx && m.matchIdx === pos) === safeIdx
        parts.push(
          <mark
            key={pos}
            style={{
              background: isCurrent ? '#fbbf24' : '#fef9c3',
              borderRadius: 2,
              padding: '0 1px',
            }}
          >
            {line.slice(pos, pos + query.length)}
          </mark>
        )
          ...
          {totalMatches > 0 && (
            <>
              <span style={{ fontSize: 11, color: '#64748b', whiteSpace: 'nowrap' }}>
                {safeIdx + 1}/{totalMatches}
              </span>
              <button
                onClick={() => setCursor((c) => (c - 1 + totalMatches) % totalMatches)}
                style={{
                  padding: '4px 8px',
                  borderRadius: 6,
                  border: '1px solid #e2e8f0',
              ...
              <button
                onClick={() => setCursor((c) => (c + 1) % totalMatches)}
                style={{
                  padding: '4px 8px',
                  borderRadius: 6,
                  border: '1px solid #e2e8f0',
        ...
        <div style={{ border: '1px solid #e2e8f0', borderRadius: 10, overflow: 'hidden' }}>
          {CONTENT_139.map((line, idx) => (
            <div
              ...
              style={{
                padding: '10px 14px',
                borderBottom: idx < CONTENT_139.length - 1 ? '1px solid #f1f5f9' : 'none',
                fontSize: 13,
                lineHeight: 1.7,
                color: '#0f172a',
              }}
...
export const WorkspaceEditor_블록_타입_검색: Story = {
  ...
  render: function WorkspaceEditorBlockSearchRender() {
    ...
    const filtered = query
      ? WORKSPACE_BLOCKS_139.filter(
          (b) => b.label.includes(query) || b.desc.includes(query) || b.type.includes(query)
        )
            ...
            filtered.map((b) => (
              <div
                ...
                style={{
                  ...
                  padding: '8px 12px',
                  cursor: 'pointer',
                  background: selected === b.type ? '#f0f9ff' : '#fff',
                  borderBottom: '1px solid #f8fafc',
              ...
              >
                ...
                <div>
                  <div
                    style={{
                      fontSize: 13,
                      fontWeight: selected === b.type ? 700 : 500,
                      color: '#0f172a',
                    }}
                  >
                    {b.label}
                ...
                {selected === b.type && (
                  <span style={{ marginLeft: 'auto', fontSize: 12, color: '#0ea5e9' }}>✓</span>
                )}
              </div>
            ))
          )}
...
const GLOBAL_DOCS_139 = [
  ...
  {
    id: 'd4',
    title: 'Carousel setApi 패턴',
    path: 'eclipse/Data Display/Carousel',
    type: '컴포넌트',
  },
]
          ...
          {!query ? (
            ...
            <>
              {filteredDocs.length > 0 ? (
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: 24,
                          borderRadius: 6,
                          background: d.type === '컴포넌트' ? '#eef2ff' : '#f0fdf4',
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center',
                          fontSize: 10,
                          fontWeight: 700,
                          color: d.type === '컴포넌트' ? '#6366f1' : '#16a34a',
                        }}
                      >
                        {d.type === '컴포넌트' ? 'C' : 'D'}
                      </div>
                ...
                <div
                  style={{ padding: '20px', textAlign: 'center', color: '#94a3b8', fontSize: 12 }}
                >
...
function M3SearchWithChipsRender() {
  ...
  const filtered = M3_COMPONENT_LIST.filter((c) => {
    const matchQuery =
      !query || c.name.toLowerCase().includes(query.toLowerCase()) || c.desc.includes(query)
          ...
          {M3_CATEGORIES.map((cat) => (
            <button
              ...
              style={{
                padding: '4px 12px',
                borderRadius: 20,
                border: `1.5px solid ${activeCategory === cat ? '#6366f1' : '#e5e7eb'}`,
                background: activeCategory === cat ? '#eef2ff' : 'transparent',
                color: activeCategory === cat ? '#4338ca' : '#64748b',
                fontSize: 11,
                cursor: 'pointer',
...
export const Material3_검색_칩_필터_패턴: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'RoleToken Design Search + Suggestion Chip + Filter Chip 패턴. 검색어 입력 전 제안 칩 표시, 카테고리 필터 칩(Filter Chip), 검색 결과 + Chip 배지 조합. M3 색상 역할 시스템 적용.',
      },
    },
  },
...
const UTILITYUI_SCOPE_RESULTS: Record<
  string,
...
function ComposableUIGlobalSearchRender() {
  ...
  const results = (UTILITYUI_SCOPE_RESULTS[scope] ?? []).filter(
    (r) =>
      !query ||
          ...
          {UTILITYUI_SEARCH_SCOPES.map((s) => (
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                color: scope === s ? '#6366f1' : '#64748b',
                fontWeight: scope === s ? 700 : 400,
                borderBottom: scope === s ? '2px solid #6366f1' : '2px solid transparent',
                transition: 'all 150ms',
              }}
            >
              {s}
...
export const ComposableUI_전역_검색_스코프_선택: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'ComposableUI 전역 검색 패턴. SearchBar + 스코프 탭(전체/컴포넌트/문서/예시) 조합. 탭 전환으로 검색 스코프 변경, 스코프별 결과 아이콘 차별화. ComposableUI 공식 문서 검색 UX 모방.',
      },
    },
  },
...
function M3ComposableUISearchHistoryRender() {
  ...
  const suggestions = query
    ? M3_COMPONENT_LIST.filter((c) => c.name.toLowerCase().startsWith(query.toLowerCase())).slice(
        0,
        5
      )
    : []
  ...
  return (
    <div style={{ width: 380, fontFamily: 'system-ui, sans-serif' }}>
      ...
      >
        RoleToken Design + ComposableUI — 검색 히스토리 관리
      </p>
      <div style={{ position: 'relative' }}>
        <SearchBar
                ...
                {suggestions.map((c) => (
                  ...
                  >
                    ...
                    <span style={{ fontSize: 12, color: '#1e293b' }}>
                      <strong style={{ color: '#6366f1' }}>{c.name.slice(0, query.length)}</strong>
                      {c.name.slice(query.length)}
                    </span>
                  </button>
                ))}
              </div>
...
export const M3_ComposableUI_검색_히스토리_관리: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'RoleToken Design 검색 트렌딩 + ComposableUI 자동완성 패턴 결합. 포커스 시 트렌딩/히스토리 드롭다운, 입력 시 자동완성(매칭 텍스트 강조), 히스토리 삭제. 검색 경험의 3단계(트렌딩→자동완성→히스토리 관리) 완성.',
      },
    },
  },
