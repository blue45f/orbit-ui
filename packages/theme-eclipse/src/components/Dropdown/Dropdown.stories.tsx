      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {filtered.map((task) => (
          ...
          >
            ...
            <span
              style={{ fontSize: '12px', color: '#64748b', minWidth: '48px', textAlign: 'right' }}
            >
        ...
        >
          ...
          >
            ...
            <ChevronRightLineIcon
              width={12}
              height={12}
              style={{ color: '#94a3b8', transform: 'rotate(90deg)' }}
            />
          </div>
        </div>
      </div>
...
export const IssueTracker_이슈_상태_선택기: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'IssueTracker 이슈 상태 선택 패턴. 색상 도트 + 상태 레이블로 현재 상태를 표시. 상태별 시맨틱 색상으로 한눈에 진행 상황 파악. 컴팩트 밀도의 상태 드롭다운.',
      },
    },
  },
  render: function IssueTrackerStatusSelector() {
    ...
    return (
      ...
      >
        ...
        {/* 드롭다운 트리거 */}
        <div style={{ position: 'relative' }}>
          <Dropdown value={current.label} activated={false} onClick={() => {}}>
            <Dropdown.Leading>
            ...
            {TRACKER_STATUSES.map((s) => (
              <div
                ...
                style={{
                  ...
                  padding: '8px 14px',
                  cursor: 'pointer',
                  background: status === s.value ? '#f8fafc' : '#fff',
                  borderLeft: `3px solid ${status === s.value ? s.color : 'transparent'}`,
                  transition: 'all 0.1s',
                }}
              >
                <span style={{ fontSize: 13, color: s.color, minWidth: 16, textAlign: 'center' }}>
                  {s.dot}
                </span>
                <span
                  style={{
                    fontSize: 13,
                    color: status === s.value ? '#0f172a' : '#374151',
...
export const IssueTracker_우선순위_선택_드롭다운: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'IssueTracker 이슈 우선순위 선택 패턴. 긴급/높음/보통/낮음/없음 5단계를 아이콘+색상으로 구분. Dropdown.Leading에 우선순위 아이콘 배치.',
      },
    },
  },
      ...
      >
        ...
        <div style={{ position: 'relative' }}>
          <div onClick={() => setOpen((o) => !o)}>
            <Dropdown value={current.label} activated={open} onClick={() => {}}>
              <Dropdown.Leading>
                <span
                  style={{
                    fontSize: 12,
              ...
              {TRACKER_PRIORITIES.map((p) => (
                ...
                >
                  ...
                  <span
                    style={{
                      fontSize: 13,
                      color: priority === p.value ? '#0f172a' : '#374151',
...
export const Material3_Exposed_드롭다운_메뉴: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Material Design 3 Exposed Dropdown Menu 패턴. 아웃라인 필드에 chevron 트리거를 조합. 선택된 값이 필드 내부에 표시. 폼 내 정렬/분류 선택에 적합.',
      },
    },
  },
        ...
        <div style={{ position: 'relative' }}>
          ...
          >
            <div>
              {sort ? (
                <>
                  <div style={{ fontSize: 10, fontWeight: 600, color: '#6366f1', marginBottom: 2 }}>
                    정렬 기준
                  </div>
                ...
                {parentItem.children?.map((child) => (
                  <div
                    ...
                    style={{
                      ...
                      fontSize: 13,
                      background:
                        selected?.child === child.value && selected?.parent === parentItem.value
                          ? '#eff6ff'
                          : '#fff',
                      color:
                        selected?.child === child.value && selected?.parent === parentItem.value
...
export const UtilityCSSUI_인원_배정_드롭다운: Story = {
  ...
  render: function TwAssigneeDropdown() {
    ...
    const filtered = TW_ASSIGNEES.filter(
      (a) => a.name.includes(query) || a.role.toLowerCase().includes(query.toLowerCase())
    )
          ...
          >
            ...
            {filtered.map((a) => (
              ...
              >
                ...
                {assignee?.id === a.id && (
                  <span style={{ marginLeft: 'auto', fontSize: 12, color: '#6366f1' }}>✓</span>
                )}
              </div>
            ))}
            {assignee && (
  ...
  render: function DeployPlatformEnvSelectorRender() {
    ...
    return (
      <div style={{ width: 320, fontFamily: 'system-ui, sans-serif', position: 'relative' }}>
        ...
        <Dropdown value={current.label} activated={open} onClick={() => setOpen((v) => !v)}>
          <Dropdown.Leading>
            ...
            {DEPLOY_ENVS_138.map((e) => (
              <div
                ...
                style={{
                  ...
                  padding: '10px 14px',
                  cursor: 'pointer',
                  background: env === e.value ? '#f8fafc' : '#fff',
                  borderBottom: '1px solid #f1f5f9',
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span
                      style={{
                        fontSize: 13,
                        fontWeight: env === e.value ? 700 : 500,
                        color: '#0f172a',
                      }}
                    >
                      {e.label}
    ...
    return (
      <div style={{ width: 340, fontFamily: 'system-ui, sans-serif', position: 'relative' }}>
        ...
        >
          <Dropdown.Trailing>
            <span
              style={{ fontSize: 11, fontWeight: 600, color: getLatencyColor(current.latency) }}
            >
              {current.latency}ms
            </span>
          </Dropdown.Trailing>
              ...
              .map((r) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '9px 12px',
                    cursor: 'pointer',
                    background: region === r.value ? '#f8fafc' : '#fff',
                    borderBottom: '1px solid #f8fafc',
                    transition: 'background 100ms',
                  }}
                >
                  <div style={{ flex: 1 }}>
                    <div
                      style={{
                        fontSize: 13,
                        fontWeight: region === r.value ? 700 : 400,
                        color: '#0f172a',
                      }}
                    >
                      {r.label}
              ...
              TEAMS_138.map((t) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '10px 12px',
                    cursor: 'pointer',
                    background: teamId === t.id ? '#f8fafc' : '#fff',
                    borderBottom: '1px solid #f8fafc',
                    transition: 'background 100ms',
                  }}
                >
              ...
              PROJECTS_138[teamId]?.map((p) => (
                <div
                  ...
                  style={{
                    ...
                    padding: '10px 12px',
                    cursor: 'pointer',
                    background: projectId === p.id ? '#f8fafc' : '#fff',
                    borderBottom: '1px solid #f8fafc',
                    transition: 'background 100ms',
                  }}
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div
                      style={{
                        fontSize: 13,
                        fontWeight: projectId === p.id ? 700 : 400,
                        color: '#0f172a',
                      }}
                    >
                      {p.name}
...
function DesignToolFrameSelectorRender() {
  ...
  return (
    <div style={{ width: 320, fontFamily: 'system-ui, sans-serif' }}>
      ...
      >
        DesignTool Plugin UI — 프레임 선택 드롭다운
      </p>
            ...
            {DESIGN_FRAME_OPTIONS.map((opt) => (
              <button
                ...
                style={{
                  ...
                  justifyContent: 'space-between',
                  padding: '9px 12px',
                  background: frame === opt ? '#3a3a3a' : 'transparent',
                  border: 'none',
                  color: frame === opt ? '#818cf8' : '#d4d4d4',
                  fontSize: 12,
                  cursor: 'pointer',
                  textAlign: 'left',
                }}
...
export const DesignTool_프레임_크기_선택_드롭다운: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'DesignTool Plugin의 Frame Size Picker 패턴. 다크 배경 플러그인 UI에 Dropdown 배치, 기기명 + 해상도 2단 표시, 선택 시 강조 색상 피드백.',
      },
    },
  },
        ...
        <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div>
            ...
            <Dropdown
              value={
                selectedFormat
                  ? `${selectedFormat.icon} ${selectedFormat.label} — ${selectedFormat.desc}`
                  : ''
              }
                ...
                {NATIVE_EXPORT_FORMATS.map((f) => (
                  <button
                    ...
                    style={{
                      ...
                      gap: 10,
                      padding: '10px 14px',
                      background: format === f.value ? '#f5f4ff' : 'transparent',
                      border: 'none',
                      cursor: 'pointer',
                      textAlign: 'left',
                      borderBottom: '1px solid #f5f5f7',
                  ...
                  >
                    ...
                    <div>
                      <div
                        style={{
                          fontSize: 13,
                          fontWeight: 600,
                          color: format === f.value ? '#5e5adb' : '#1c1c1e',
                        }}
                      >
                        {f.label}
                      </div>
                    ...
                    {format === f.value && (
                      <span style={{ marginLeft: 'auto', fontSize: 14, color: '#5e5adb' }}>✓</span>
                    )}
                  </button>
                ))}
              </div>
              ...
              {NATIVE_SCALE_OPTIONS.map((s) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 8,
                    border: `1.5px solid ${scale === s ? '#5e5adb' : '#e5e7eb'}`,
                    background: scale === s ? '#f5f4ff' : '#fff',
                    color: scale === s ? '#5e5adb' : '#1c1c1e',
                    cursor: 'pointer',
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 10,
              border: 'none',
              background: exported ? '#22c55e' : format ? '#007aff' : '#e5e7eb',
              color: format ? '#fff' : '#8e8e93',
...
export const Platform_내보내기_형식_선택_드롭다운: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Platform HIG Share Sheet / Export 패턴. 형식(PNG/SVG/PDF/WebP) + 해상도(1x/2x/3x) Dropdown 조합. iOS 스타일 둥근 카드, 큰 내보내기 버튼, 완료 피드백 애니메이션.',
      },
    },
  },
...
function DesignToolPlatformTypographyPickerRender() {
  ...
  return (
    <div style={{ width: 340, fontFamily: 'system-ui, sans-serif' }}>
      ...
      >
        DesignTool + Platform HIG — 폰트 선택기
      </p>
      <div
        style={{
          background: '#fff',
        ...
        >
          ...
          {fontFamily && (
            <div style={{ fontSize: 10, color: '#94a3b8', marginTop: 4 }}>
              {fontSize}px · The quick brown fox
            </div>
          )}
        </div>
                ...
                {DESIGN_FONT_FAMILIES.map((f) => (
                  <button
                    ...
                    style={{
                      width: '100%',
                      padding: '8px 12px',
                      background: fontFamily === f.name ? '#f0f4ff' : 'transparent',
                      border: 'none',
                      textAlign: 'left',
                      cursor: 'pointer',
                      fontSize: 13,
                      color: fontFamily === f.name ? '#4338ca' : '#374151',
                      fontFamily: f.name,
                    }}
                  >
                    {f.name}
                ...
                {selectedFont.weights.map((w) => (
                  <button
                    ...
                    style={{
                      width: '100%',
                      padding: '8px 12px',
                      background: fontWeight === w ? '#f0f4ff' : 'transparent',
                      border: 'none',
                      textAlign: 'left',
                      cursor: 'pointer',
                      fontSize: 13,
                      color: fontWeight === w ? '#4338ca' : '#374151',
                    }}
                  >
                    {w}
                  </button>
            ...
            >
              {FONT_SIZES.map((s) => (
                <button
                  ...
                  style={{
                    padding: '8px 0',
                    background: fontSize === s ? '#f0f4ff' : 'transparent',
                    border: 'none',
                    cursor: 'pointer',
                    fontSize: 12,
                    color: fontSize === s ? '#4338ca' : '#374151',
...
export const DesignTool_Platform_폰트_타이포그래피_선택기: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'DesignTool Font Picker + Platform HIG Typography Inspector 패턴. 패밀리/굵기/크기 3개 Dropdown 연동, 실시간 폰트 미리보기. 패밀리 선택 시 굵기 옵션 동적 업데이트.',
      },
    },
  },
