...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'PageIndicator는 슬라이더, 캐러셀, 스텝 위자드의 현재 위치를 dot으로 표시하는 컴포넌트입니다. ComposableUI Steps, EnterpriseUI Stepper 패턴을 지원합니다.',
      },
    },
  },
  argTypes: {
...
const WIZARD_STEPS = [
  {
    title: '프로젝트 정보',
    desc: '기본 정보를 입력하세요',
    content: '프로젝트 이름, 설명, 카테고리를 설정합니다.',
  },
  {
    title: '팀 구성',
    desc: '팀원을 초대하세요',
    content: '이메일을 입력해 팀원을 초대합니다. 역할을 지정할 수 있습니다.',
  },
  {
    title: '권한 설정',
    desc: '접근 수준을 정하세요',
    content: '각 팀원의 읽기/쓰기/관리자 권한을 설정합니다.',
  },
  {
    title: '확인',
    desc: '설정을 검토하세요',
    content: '모든 설정을 확인하고 프로젝트를 생성합니다.',
  },
]
  ...
  return (
    ...
    >
      <div style={{ padding: '40px 32px 24px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 13, color: '#64748b', lineHeight: 1.7 }}>{feature.desc}</div>
      </div>
      <div
        style={{
      ...
      >
        ...
        <div style={{ fontSize: 10, color: '#3a3a4e' }}>← → 키로 탐색</div>
      </div>
    </div>
  )
}
...
const GALLERY_SLIDES = [
  {
    bg: '#eff6ff',
    title: 'Button 컴포넌트',
    sub: '5가지 variant · 3가지 사이즈',
    accent: '#6366f1',
  },
  {
    bg: '#f0fdf4',
    title: 'Toggle & Switch',
    sub: '접근성 완벽 지원 · onCheckedChange',
    accent: '#10b981',
  },
  { bg: '#fffbeb', title: 'Data Table', sub: '정렬 · 필터 · 페이지네이션', accent: '#f59e0b' },
  {
    ...
    sub: 'TextField · Checkbox · RadioButton',
    accent: '#8b5cf6',
  },
]
  ...
  return (
    <div style={{ width: 480, fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <PageIndicator currentPage={step} onPageChange={(p) => setStep(p)}>
...
const DeployPlatformDeploymentIndicatorRender = () => {
  ...
  return (
    <div style={{ width: 440, fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ marginBottom: 12, fontSize: 13, fontWeight: 700, color: '#0f172a' }}>
        배포 히스토리
      </div>
      <div
        style={{
          ...
          >
            <div
              style={{
                ...
                boxShadow:
                  d.status === 'building' ? `0 0 0 3px ${STATUS_CFG.building.dot}30` : 'none',
              }}
            />
            <div style={{ flex: 1 }}>
      ...
      >
        ...
        <span style={{ color: '#94a3b8' }}>
          {' '}
          · {STATUS_CFG[active.status as DeployStatus].label}
        </span>
      </div>
...
const ComposableUIDeployPlatformProgressRender = () => {
  ...
  return (
    <div style={{ width: 440, fontFamily: 'system-ui, sans-serif' }}>
            ...
            <React.Fragment key={i}>
              ...
              >
                ...
                <div
                  style={{
                    fontSize: 10,
                    color: i <= currentStep ? p.color : '#94a3b8',
      ...
      >
        {running
          ? `${PIPELINE[currentStep].label} 진행 중...`
          : currentStep === PIPELINE.length - 1
            ? '모든 단계 완료'
            : `${PIPELINE[currentStep].label} 단계`}
      </div>
...
const SURVEY_STEPS = [
  {
    q: 'Orbit UI를 어떤 용도로 사용하시나요?',
    opts: ['개인 프로젝트', '팀 프로젝트', '프리랜서 작업', '기업 서비스'],
  },
  {
    q: '현재 사용 중인 다른 디자인 시스템은?',
    opts: ['ComposableUI', 'EnterpriseUI', 'Ant Design', '직접 구현'],
  },
  {
    q: '가장 중요한 기능은 무엇인가요?',
    opts: ['접근성', '커스터마이징', '문서 품질', '컴포넌트 수'],
  },
  ...
  return (
    <div style={{ width: 360, display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {current.opts.map((opt) => (
          <button
            ...
            style={{
              padding: '12px 16px',
              borderRadius: 8,
              border: `1.5px solid ${answers[step] === opt ? '#6366f1' : '#e2e8f0'}`,
              background: answers[step] === opt ? '#6366f108' : '#fff',
              fontSize: 13,
              color: '#0f172a',
              cursor: 'pointer',
              textAlign: 'left',
              transition: 'all 0.15s',
              fontWeight: answers[step] === opt ? 600 : 400,
            }}
          >
            {opt}
          </button>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        <button
          onClick={() => setStep((s) => Math.min(SURVEY_STEPS.length - 1, s + 1))}
          style={{
            padding: '6px 14px',
            borderRadius: 6,
            border: '1px solid #e2e8f0',
      ...
      <div style={{ fontSize: 11, color: '#94a3b8', textAlign: 'center' }}>
        ComposableUI 설문 위자드 패턴 — PageIndicator 스텝 탐색
      </div>
    </div>
  )
}
...
export const ComposableUI_설문_스텝_위자드: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'ComposableUI Steps 패턴의 설문 위자드. PageIndicator로 현재 단계를 표시하고, 답변 선택 시 자동으로 다음 단계로 이동합니다. 마지막 단계에서 완료 화면으로 전환.',
      },
    },
  },
  ...
  return (
    <div style={{ width: 380, display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
        {TRACKER_MILESTONES.map((ml, i) => (
          <React.Fragment key={ml.name}>
            ...
            >
              <div
                style={{
                  ...
                  height: 24,
                  borderRadius: '50%',
                  border: `2px solid ${ml.status === 'done' ? '#10b981' : ml.status === 'current' ? '#6366f1' : '#e2e8f0'}`,
                  background: ml.status === 'done' ? '#10b981' : active === i ? '#6366f1' : '#fff',
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                  transition: 'all 0.15s',
            ...
            {i < TRACKER_MILESTONES.length - 1 && (
              <div
                style={{
                  flex: 1,
                  height: 2,
                  background: i < active ? '#10b981' : '#e2e8f0',
                  transition: 'background 0.3s',
                  marginBottom: 16,
                }}
              />
      ...
      >
        ...
        >
          <div
            style={{
              ...
              borderRadius: 3,
              background:
                m.status === 'done' ? '#10b981' : m.status === 'current' ? '#6366f1' : '#e2e8f0',
        ...
        >
          <span>
            {m.done} / {m.tasks} 태스크 완료
          </span>
          <span style={{ fontWeight: 700, color: m.status === 'done' ? '#10b981' : '#6366f1' }}>
            {pct}%
          </span>
        </div>
      </div>
      <div style={{ fontSize: 11, color: '#94a3b8', textAlign: 'center' }}>
        IssueTracker 마일스톤 진행 패턴 — 분기별 로드맵 + PageIndicator
      </div>
    </div>
  )
}
...
export const IssueTracker_마일스톤_진행_인디케이터: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'IssueTracker 로드맵 마일스톤 UI에서 영감을 받은 패턴. 분기별 마일스톤 타임라인 + PageIndicator 동기화, 완료/진행중/예정 상태 시각화, 선택된 마일스톤 태스크 진행률 표시.',
      },
    },
  },
  ...
  return (
    ...
    >
            ...
            <div style={{ flex: 1, textAlign: 'center' }}>
              <div
                style={{
                  ...
                  color: stepIndex >= i ? '#6366f1' : '#94a3b8',
      ...
      <PageIndicator
        ...
        onPageChange={(idx) => setCurrentStep(UPLOAD_STEPS[idx].step)}
      >
      ...
      {currentStep === 'done' && (
        ...
        >
          ...
          <div style={{ fontSize: 12, color: '#64748b' }}>
            orbit-logo.svg가 성공적으로 업로드되었습니다.
          </div>
          <button
            onClick={reset}
            style={{
      ...
      <div style={{ fontSize: 11, color: '#94a3b8', textAlign: 'center' }}>
        ComposableUI + IssueTracker — 파일 업로드 스텝 위자드 + PageIndicator
      </div>
    </div>
  )
}
...
export const ComposableUI_IssueTracker_파일_업로드_스텝: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'ComposableUI Dropzone + IssueTracker 진행률 패턴 조합. PageIndicator로 선택→미리보기→완료 3단계를 표시하고, 업로드 진행률 바와 단계 전환 애니메이션을 연출합니다.',
      },
    },
  },
...
function PlatformHIGPageControl142Render() {
  ...
  const slides = [
    {
      title: '모든 기기에서 이어서',
      desc: 'iPhone, iPad, Mac 어디서나 끊김없이 이어서 작업하세요. iCloud가 자동으로 동기화합니다.',
      bg: '#1d4ed8',
    },
    {
      title: '집중 모드',
      desc: '업무 집중, 개인 시간, 취침 등 상황에 맞게 알림을 자동으로 조절하세요.',
      bg: '#7c3aed',
    },
    {
      title: '프라이버시 우선',
      desc: '앱이 카메라, 마이크, 위치에 접근할 때 언제나 투명하게 알려드립니다.',
      bg: '#0f766e',
    },
    {
      title: '시작하기',
      desc: '지금 바로 시작해보세요. 설정은 언제든지 변경할 수 있습니다.',
      bg: '#b45309',
    },
  ]
  ...
  return (
    <div style={{ width: 360, fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
        ...
        <div style={{ fontSize: 11, color: '#94a3b8' }}>
          Platform HIG — Page Control 온보딩 패턴
        </div>
      </div>
    </div>
  )
...
export const Platform_HIG_온보딩_페이지_컨트롤: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Platform HIG Page Control 온보딩 패턴. 슬라이드 배경색 전환, dot 클릭 직접 이동, 이전/다음 버튼. 마지막 슬라이드 전 버튼 "시작"으로 전환.',
      },
    },
  },
  ...
  return (
    <div style={{ width: 380, fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ marginBottom: 10, fontSize: 11, color: '#64748b' }}>
        RoleToken Design — Stepper + PageIndicator
      </div>
            ...
            >
              <div
                style={{
                  ...
                  fontSize: 12,
                  fontWeight: 700,
                  background: completed[i] ? '#10b981' : i === step ? '#6750a4' : '#f1f5f9',
                  color: completed[i] || i === step ? '#fff' : '#94a3b8',
                  transition: 'all 0.2s',
                  border: `2px solid ${i === step ? '#6750a4' : 'transparent'}`,
                }}
              >
                {completed[i] ? '✓' : i + 1}
              </div>
            ...
            {i < steps.length - 1 && (
              <div
                style={{
                  flex: 1,
                  height: 2,
                  background: completed[i] ? '#10b981' : '#e2e8f0',
                  margin: '0 4px',
                  marginBottom: 14,
                  transition: 'background 0.3s',
                }}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 20,
            border: 'none',
            background: step >= steps.length - 1 ? '#e2e8f0' : '#6750a4',
            color: step >= steps.length - 1 ? '#94a3b8' : '#fff',
            fontSize: 13,
            fontWeight: 600,
            cursor: step >= steps.length - 1 ? 'not-allowed' : 'pointer',
...
export const M3_스텝퍼_인디케이터: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'RoleToken Design Stepper 패턴 + PageIndicator 동기화. 완료 단계 체크 표시, M3 보라색 팔레트, 스텝 연결선 완료 시 그린 전환.',
      },
    },
  },
...
function PlatformM3MediaCarousel142Render() {
  ...
  return (
    <div style={{ width: 340, fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ marginBottom: 10, fontSize: 11, color: '#64748b' }}>
        Platform HIG + M3 — 미디어 캐러셀 PageIndicator
      </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 8,
                border: 'none',
                background: autoPlay ? item.color : '#f1f5f9',
                fontSize: 12,
                cursor: 'pointer',
                color: autoPlay ? '#fff' : '#374151',
...
export const Platform_M3_미디어_캐러셀_인디케이터: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Platform HIG 카드 캐러셀 + M3 색상 역할 시스템. 자동 재생 토글, PageIndicator dot 직접 클릭, 아이템 색상 동적 변환 (배경/버튼).',
      },
    },
  },
...
export const Accessible_튜토리얼_진행_인디케이터: Story = {
  ...
  render: function AccessibleTutorialIndicatorRender() {
    ...
    return (
      <div style={{ width: 380, fontFamily: 'system-ui, sans-serif' }}>
        <div
          style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 14, padding: 24 }}
        >
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 8,
                border: 'none',
                background: current === steps.length - 1 ? '#e5e7eb' : '#3b82f6',
                color: current === steps.length - 1 ? '#9ca3af' : '#fff',
                fontSize: 12,
                fontWeight: 600,
...
export const DataProduct_이미지_뷰어_페이지_닷: Story = {
  ...
  render: function DataProductImageViewerRender() {
    const slides = [
      ...
      {
        label: 'DeployPlatform 자동 배포',
        bg: 'linear-gradient(135deg, #f59e0b, #10b981)',
    ...
    return (
      <div style={{ width: 360, fontFamily: 'system-ui, sans-serif' }}>
        ...
        >
          ...
          <span
            style={{ color: autoPlay ? '#10b981' : '#9ca3af', fontWeight: autoPlay ? 600 : 400 }}
          >
        ...
        {isDone ? (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <button
              onClick={() => {
                setStep(0)
                setForm({ name: '', company: '', role: '', useCase: '' })
              }}
              style={{
                marginTop: 14,
                padding: '8px 20px',
          ...
          <>
            <div style={{ marginBottom: 20 }}>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((prev) => ({ ...prev, [steps[step].field]: e.target.value }))
                }
                placeholder={steps[step].placeholder}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && canNext) setStep((p) => p + 1)
                }}
                style={{
                  width: '100%',
                  padding: '10px 12px',
