    ...
    >
      ...
      >
        <Breadcrumb>
          <Breadcrumb.List>
            ...
            <Breadcrumb.Item>
              <Breadcrumb.Link href="/design" style={{ color: '#64748b', fontSize: '13px' }}>
                문서
              </Breadcrumb.Link>
            </Breadcrumb.Item>
            <Breadcrumb.Separator>
            ...
            <Breadcrumb.Item>
              <Breadcrumb.Page style={{ fontSize: '13px', color: '#1e293b', fontWeight: 500 }}>
                컴포넌트 가이드
              </Breadcrumb.Page>
            </Breadcrumb.Item>
          </Breadcrumb.List>
      ...
      <div style={{ padding: '48px 24px 24px' }}>
        ...
        <p style={{ fontSize: '15px', color: '#64748b', margin: 0, lineHeight: 1.6 }}>
          Orbit UI의 컴포넌트를 활용하는 방법과 모범 사례를 소개합니다. 각 컴포넌트의 Props, 변형,
          접근성 지침을 확인하세요.
        </p>
      </div>
    </div>
...
const BreadcrumbNavigator = () => {
  ...
  const hasNext = tree.some(
    (n) => n.parent === currentPath[currentPath.length - 1] && !currentPath.includes(n.id)
  )
    ...
    return (
      <div style={{ maxWidth: 560 }}>
          ...
          >
            <button
              ...
              style={{
                ...
                background: 'transparent',
                fontSize: 12,
                cursor: pathIdx === 0 ? 'not-allowed' : 'pointer',
                color: pathIdx === 0 ? '#cbd5e1' : '#475569',
              }}
            >
              ←
            </button>
            <button
              ...
              style={{
                ...
                background: 'transparent',
                fontSize: 12,
                cursor: pathIdx === FS_PATHS.length - 1 ? 'not-allowed' : 'pointer',
                color: pathIdx === FS_PATHS.length - 1 ? '#cbd5e1' : '#475569',
              }}
            >
              →
            </button>
                ...
                <Breadcrumb.Item key={item.label}>
                  <Breadcrumb.Link
                    ...
                    style={{
                      cursor: 'pointer',
                      fontSize: 12,
                      color: i === currentPath.length - 1 ? '#0f172a' : '#64748b',
        ...
        <p style={{ marginTop: 8, fontSize: 11, color: '#94a3b8' }}>
          UtilityUI — 파일 탐색기 스타일 Breadcrumb 패턴
        </p>
      </div>
    )
  },
...
const SETTINGS_TREE = {
  ...
  children: {
    ...
    팀: {
      label: '팀 설정',
      children: { 멤버: { label: '멤버 관리' }, 권한: { label: '권한 설정' } },
    },
    결제: {
      label: '결제 & 플랜',
      children: { 청구: { label: '청구 정보' }, 히스토리: { label: '결제 기록' } },
    },
  },
}
      ...
      <div style={{ maxWidth: 400 }}>
        ...
        <Breadcrumb style={{ marginBottom: 16 }}>
          <Breadcrumb.Item>
            <Breadcrumb.Link
              onClick={() => setPath([])}
              style={{ cursor: 'pointer', fontSize: 13, color: '#6366f1', fontWeight: 500 }}
            >
              설정
            </Breadcrumb.Link>
          </Breadcrumb.Item>
            ...
            <Breadcrumb.Item key={key}>
              ...
              <Breadcrumb.Link
                ...
                style={{
                  cursor: 'pointer',
                  fontSize: 13,
                  color: i === path.length - 1 ? '#0f172a' : '#6366f1',
        ...
        <div style={{ borderRadius: 10, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
          ...
          {hasChildren ? (
            ...
            <div
              style={{ padding: '24px 16px', textAlign: 'center', color: '#94a3b8', fontSize: 13 }}
            >
        ...
        <p style={{ marginTop: 8, fontSize: 11, color: '#94a3b8' }}>
          PrimitiveUI Breadcrumb primitive — 계층형 설정 탐색 패턴
        </p>
      </div>
    )
  },
...
const NATIVE_FS: PlatformFolder = {
  ...
  items: [
    ...
    },
    {
      name: 'Desktop',
      items: [
        { name: 'Screenshots', items: [] },
        { name: 'Recordings', items: [] },
      ],
    },
  ],
}
                ...
                <Breadcrumb.Item key={name}>
                  {i < breadcrumb.length - 1 ? (
                    <Breadcrumb.Link
                      ...
                      onClick={(e) => {
                        e.preventDefault()
                        setPath(breadcrumb.slice(1, i + 1))
                      }}
      ...
      <div style={{ padding: 8, display: 'flex', flexDirection: 'column', gap: 2 }}>
        {current.items.length > 0 ? (
          current.items.map((folder) => (
            ...
            >
              <span style={{ fontSize: 18 }}>{folder.items.length > 0 ? '📁' : '📄'}</span>
              <span style={{ flex: 1 }}>{folder.name}</span>
              {folder.items.length > 0 && <ChevronRightLineIcon size={12} />}
            </div>
          ))
        ) : (
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '16px 20px 0' }}>
        ...
        <div
          style={{ padding: '8px 12px', background: '#f8fafc', borderRadius: 8, marginBottom: 12 }}
        >
          <Breadcrumb>
            <Breadcrumb.List>
              <Breadcrumb.Item>
      ...
      <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 2 }}>
        {section.items.map((item, i) => (
          ...
          >
            ...
            {item.detail && SETTING_SECTIONS[item.label] && <ChevronRightLineIcon size={12} />}
          </div>
        ))}
      </div>
      <div style={{ padding: '8px 16px', fontSize: 11, color: '#8e8e93' }}>
  ...
  render: () => (
    <div style={{ maxWidth: 560 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
        {['MacBook Pro 16"', 'LG 그램 17', 'Samsung Book Pro', 'Lenovo ThinkPad'].map(
          (product, i) => (
            <div
              key={product}
              style={{
      ...
      <p style={{ marginTop: 8, fontSize: 11, color: '#94a3b8' }}>
        ComposableUI Breadcrumb — 전자상거래 카테고리 탐색 + 공유 버튼 패턴
      </p>
    </div>
  ),
}
          ...
          {presets.map((p) => (
            <button
              ...
              style={{
                ...
                padding: '6px 10px',
                borderRadius: 6,
                border: `1px solid ${JSON.stringify(p) === JSON.stringify(path) ? '#6366f1' : 'var(--sem-eclipse-color-borderSubtle)'}`,
                background:
            ...
            {ws.views.map((v) => (
              <button
                ...
                style={{
                  ...
                  background: 'none',
                  border: 'none',
                  borderBottom: `2px solid ${activeView === v ? '#6366f1' : 'transparent'}`,
                  cursor: 'pointer',
...
function DataProductDataDrilldownRender() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 16 }}>
      ...
      <Breadcrumb
        separator={<ChevronRightLineIcon style={{ width: 14, height: 14, color: '#94a3b8' }} />}
      >
        <Breadcrumb.Item>
...
function PlatformHIGNavigationRender() {
  ...
  return (
    <div style={{ width: 360, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <Breadcrumb
        separator={<ChevronRightLineIcon style={{ width: 12, height: 12, color: '#94a3b8' }} />}
      >
        {path.map((id) => (
...
function DataProductPlatformFilePathRender() {
  ...
  const actualIndices = isCollapsed
    ? [
        0,
        -1,
        ...Array.from({ length: MAX_VISIBLE - 2 }, (_, i) => path.length - MAX_VISIBLE + 2 + i),
      ]
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <button
          ...
          style={{
            ...
            color: path.length > 1 ? '#0f172a' : '#94a3b8',
          }}
        >
          ← 상위
        </button>
        <Breadcrumb separator="/">
          {visiblePath.map((segment, i) => (
            <Breadcrumb.Item
              ...
              style={{
                cursor: actualIndices[i] !== -1 ? 'pointer' : 'default',
                color: segment === '...' ? '#94a3b8' : undefined,
              }}
            >
      ...
      >
        ...
        <code style={{ fontSize: 12, color: '#475569', fontFamily: 'monospace' }}>
          /{path.join('/')}
        </code>
      </div>
  ...
  return (
    <div style={{ maxWidth: 560, fontFamily: 'system-ui, sans-serif' }}>
            ...
            <React.Fragment key={step}>
              <Breadcrumb.Item>
                {idx < currentStep ? (
                  ...
                  <Breadcrumb.Page>
                    <span
                      style={{
                        ...
                        borderBottom: '2px solid var(--sem-eclipse-color-fillPrimary)',
                        paddingBottom: 1,
                      }}
                    >
                      {step}
                  ...
                  <span
                    style={{ fontSize: 12, color: 'var(--sem-eclipse-color-foregroundDisabled)' }}
                  >
                    {step}
                  </span>
                )}
      ...
      >
        ...
        <p style={{ fontSize: 11, color: 'var(--sem-eclipse-color-foregroundTertiary)' }}>
          단계 {currentStep + 1} / {steps.length} — 완료된 단계를 클릭하면 돌아갈 수 있습니다.
        </p>
      </div>
    </div>
  )
...
export const UtilityCSS_다단계_폼_진행_브레드크럼: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'UtilityUI의 step indicator + breadcrumb 조합 패턴. 완료 단계는 링크로, 현재 단계는 bold underline으로, 미래 단계는 비활성으로 표현. 완료 단계 클릭 시 해당 단계로 이동.',
      },
    },
  },
        ...
        {children.length > 0 ? (
          children.map((child) => {
            ...
            return (
              <div
                key={child}
                onClick={() => !isFile && setPath([...path, child])}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: 8,
...
export const Ant_코드_리뷰_파일_탐색: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Ant Design 파일 탐색 Breadcrumb 패턴. 폴더 클릭 시 경로 추가, 상위 경로 클릭 시 되돌아가는 인터랙티브 파일 트리 탐색. 코드 리뷰 UI에서 자주 쓰이는 패턴.',
      },
    },
  },
  ...
  return (
    <div style={{ maxWidth: 560, fontFamily: 'system-ui, sans-serif' }}>
      ...
      >
        ...
        <p style={{ fontSize: 11, color: 'var(--sem-eclipse-color-foregroundTertiary)' }}>
          사용자 역할 및 접근 권한을 관리합니다. 변경 사항은 즉시 적용됩니다.
        </p>
      </div>
    </div>
  )
...
export const EnterpriseUI_관리자_대시보드_브레드크럼: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Enterprise UI 관리자 UI의 아이콘 포함 Breadcrumb 패턴. 각 경로 항목에 섹션 아이콘을 추가하고 현재 섹션에 Admin 뱃지를 우측에 배치.',
      },
    },
  },
    ...
    >
            ...
            <Breadcrumb.Item key={page.label}>
              <button
                ...
                style={{
                  ...
                  background: 'none',
                  border: 'none',
                  cursor: i === activePath.length - 1 ? 'default' : 'pointer',
                  padding: '2px 4px',
                  borderRadius: 4,
                  color:
                    i === activePath.length - 1
                      ? 'var(--sem-eclipse-color-foregroundPrimary, #0f172a)'
                      : '#64748b',
                  fontSize: 13,
                }}
              >
      ...
      <div style={{ padding: '20px 24px' }}>
        ...
        <div style={{ fontSize: 13, color: '#64748b', lineHeight: 1.7 }}>
          WorkspaceEditor 스타일 페이지 계층 탐색. 브레드크럼 클릭으로 상위 페이지 이동, 아이콘 +
          제목 패턴.
        </div>
...
export const WorkspaceEditor_페이지_계층_브레드크럼: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'WorkspaceEditor 사이드바 페이지 계층 Breadcrumb 패턴. 아이콘 + 텍스트 조합, 클릭으로 상위 계층 이동, 현재 위치 아이콘 헤더 표시. 페이지 액션 버튼 연동.',
      },
    },
  },
    ...
    <div style={{ width: 480, background: '#1e1e1e', borderRadius: 10, overflow: 'hidden' }}>
            ...
            <Breadcrumb.Item key={layer.name}>
              <button
                ...
                style={{
                  ...
                  background: 'none',
                  border: 'none',
                  cursor: i === visible.length - 1 ? 'default' : 'pointer',
                  color: i === visible.length - 1 ? '#f2f2f2' : '#8e8e93',
                  fontSize: 11,
                  padding: '2px 4px',
                  borderRadius: 3,
                }}
              >
                <span style={{ fontSize: 10, color: typeColor[layer.type] }}>
                  {typeIcon[layer.type]}
                </span>
                {layer.name}
              </button>
      ...
      <div style={{ padding: '14px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          <span style={{ fontSize: 14, color: typeColor[layers[active].type] }}>
            {typeIcon[layers[active].type]}
          </span>
          ...
          <span
            style={{
              ...
              padding: '1px 6px',
              borderRadius: 99,
              background: `${typeColor[layers[active].type]}30`,
              color: typeColor[layers[active].type],
              fontWeight: 600,
            }}
          >
            {layers[active].type}
        ...
        <div style={{ marginTop: 12, display: 'flex', gap: 4 }}>
          {layers.slice(active + 1).map((l) => (
            <button
              key={l.name}
              onClick={() => setActive(layers.indexOf(l))}
              style={{
                padding: '2px 6px',
                borderRadius: 4,
                background: '#2c2c2c',
...
export const DesignTool_레이어_계층_브레드크럼: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'DesignTool 플러그인 레이어 탐색 Breadcrumb. Page→Frame→Group→Component→Instance 계층 표시, 타입별 컬러 아이콘, 클릭 시 선택 레이어 변경, 하위 레이어 접근 버튼.',
      },
    },
  },
    ...
    >
            ...
            <Breadcrumb.Item key={segment}>
              <button
                ...
                style={{
                  background: 'none',
                  border: 'none',
                  cursor: i === docPath.length - 1 ? 'default' : 'pointer',
                  color: i === docPath.length - 1 ? '#0f172a' : '#64748b',
                  fontSize: 12,
                  padding: '1px 4px',
                  borderRadius: 3,
                  fontWeight: i === docPath.length - 1 ? 600 : 400,
                }}
              >
                {segment}
              </button>
      ...
      <div style={{ display: 'flex', minHeight: 200 }}>
        <div
          style={{
            width: 160,
            borderRight: '1px solid var(--sem-eclipse-color-borderPrimary, #e2e8f0)',
            padding: '12px 8px',
          }}
        >
          {allSections.map((s) => (
            <button
              ...
              style={{
                ...
                padding: '5px 10px',
                borderRadius: 6,
                background: docPath.includes(s) ? '#f1f5f9' : 'none',
                border: 'none',
                fontSize: 12,
                color: docPath.includes(s) ? '#0f172a' : '#64748b',
                cursor: 'pointer',
                fontWeight: docPath.includes(s) ? 600 : 400,
                marginBottom: 2,
              }}
            >
              {s}
        ...
        <div style={{ flex: 1, padding: '14px 16px' }}>
          {subsections[currentSection] ? (
                ...
                {subsections[currentSection].map((sub) => (
                  <button
                    ...
                    style={{
                      ...
                      padding: '6px 10px',
                      borderRadius: 6,
                      background: docPath.includes(sub)
                        ? '#eff6ff'
                        : 'var(--sem-eclipse-color-surfaceContainer, #f8fafc)',
                      border: 'none',
                      fontSize: 13,
                      color: docPath.includes(sub) ? '#3b82f6' : '#475569',
                      cursor: 'pointer',
                      fontWeight: docPath.includes(sub) ? 600 : 400,
                    }}
                  >
                    {sub}
                  </button>
            ...
            <div style={{ fontSize: 13, color: '#64748b' }}>
              ...
              <br />
              WorkspaceEditor + DesignTool 스타일 브레드크럼 네비게이션 예시.
            </div>
          )}
        </div>
      </div>
...
export const WorkspaceEditor_DesignTool_문서_사이드바_브레드크럼: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'WorkspaceEditor 문서 + DesignTool 플러그인 네비게이션 패턴. 좌측 섹션 메뉴 클릭 → 브레드크럼 동적 업데이트 → 서브섹션 선택 → 3단계 경로 표시. 실제 문서 사이트 탐색 UX.',
      },
    },
  },
