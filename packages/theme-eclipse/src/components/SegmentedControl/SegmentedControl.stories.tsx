  ...
  const issueGroups: Record<string, { title: string; color: string; issues: string[] }[]> = {
    status: [
      {
        title: 'In Progress',
        color: '#f59e0b',
        issues: ['[ORB-142] Carousel 고도화', '[ORB-141] TabGroup M3 패턴'],
      },
      {
        title: 'Todo',
        color: '#6366f1',
        issues: ['[ORB-143] SearchBar 개선', '[ORB-144] Toast 접근성'],
      },
    ...
    priority: [
      { title: 'Urgent', color: '#ef4444', issues: ['[ORB-142] Carousel 고도화'] },
      {
        title: 'High',
        color: '#f59e0b',
        issues: ['[ORB-143] SearchBar 개선', '[ORB-141] TabGroup M3 패턴'],
      },
      {
        title: 'Medium',
        color: '#6366f1',
        issues: ['[ORB-144] Toast 접근성', '[ORB-140] AlertDialog 스토리'],
      },
    ],
    assignee: [
      {
        title: 'Kim Heejun',
        color: '#6366f1',
        issues: ['[ORB-142] Carousel 고도화', '[ORB-143] SearchBar 개선'],
      },
      {
        title: 'Unassigned',
        color: '#94a3b8',
        issues: ['[ORB-141] TabGroup M3 패턴', '[ORB-144] Toast 접근성'],
      },
    ],
    label: [
      {
    ...
    >
      ...
      <div
        style={{
          padding: '12px 14px',
          display: 'flex',
          flexDirection: layout === 0 ? 'column' : 'row',
          gap: 12,
          overflowX: 'auto',
        }}
      >
...
export const IssueTracker_그룹핑_레이아웃_전환: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'IssueTracker의 All Issues 뷰 패턴. Group By와 Layout 두 개의 SegmentedControl을 조합해 콘텐츠 구성 방식을 제어합니다. 하나의 UI에 복수 SegmentedControl을 사용하는 실무 패턴입니다.',
      },
    },
  },
        ...
        <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          {[
            ...
            <div key={row.label} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontSize: 13, color: '#475569', minWidth: 64, fontWeight: 500 }}>
                {row.label}
              </span>
          ...
          {['Primary', 'Secondary', 'Ghost'].map((label, i) => (
            <button
              ...
              style={{
                padding,
                borderRadius,
                border: i === 1 ? `1px solid ${isDark ? '#475569' : '#e2e8f0'}` : 'none',
...
export const ComposableUI_외관_설정_제어: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'ComposableUI의 ThemeToggle + Appearance 설정 패턴. 여러 SegmentedControl로 외관(다크/라이트), 밀도, 모서리 반경을 독립적으로 제어하고 라이브 미리보기로 즉각 피드백을 제공합니다.',
      },
    },
  },
...
export const WorkspaceEditor_인라인_블록_툴바: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'WorkspaceEditor의 인라인 툴바 패턴. SegmentedControl을 소형 아이콘/텍스트 모드로 사용해 텍스트 유형과 정렬을 한 줄에 제어합니다. 두 컨트롤이 조합되어 에디터 블록 속성을 즉시 반영합니다.',
      },
    },
  },
...
export const UtilityCSS_상품_정렬_컨트롤: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'UtilityUI의 정렬 필터 패턴. SegmentedControl로 최신/인기/가격 정렬 기준을 전환하면 아래 상품 목록이 즉시 재정렬됩니다. 컴팩트한 헤더 영역에 정렬 컨트롤을 배치합니다.',
      },
    },
  },
...
export const EnterpriseUI_차트_기간_전환: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'EnterpriseUI Tabs의 기간 선택 패턴을 SegmentedControl로 구현. 1D/1W/1M/3M/1Y 탭 전환으로 바 차트 데이터와 등락률이 즉시 업데이트됩니다. 금융/분석 대시보드에서 자주 쓰이는 패턴입니다.',
      },
    },
  },
        ...
        <div style={{ border: '1px solid #e5e5ea', borderRadius: 12, overflow: 'hidden' }}>
          {NATIVE_ITEMS.map((item, i) => (
            <div
              ...
              style={{
                padding: '13px 16px',
                borderBottom: i < NATIVE_ITEMS.length - 1 ? '1px solid #f2f2f7' : 'none',
                background: '#fff',
                fontSize: 14,
                color: '#1c1c1e',
                display: 'flex',
...
export const Platform_HIG_레이아웃_전환: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'Platform HIG의 세그먼티드 컨트롤 레이아웃 전환 패턴. 격자/목록/컴팩트 세 가지 보기 방식을 전환하면 동일 데이터가 다른 레이아웃으로 렌더링됩니다. iOS Files/Photos 앱에서 활용하는 패턴입니다.',
      },
    },
  },
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <SegmentedControl selectedIndex={modeIdx} onTabChange={setModeIdx}>
            ...
            <SegmentedControl.Tab value="grid">
              <SegmentedControl.TabCenter>격자</SegmentedControl.TabCenter>
            </SegmentedControl.Tab>
            <SegmentedControl.Tab value="kanban">
              <SegmentedControl.TabCenter>칸반</SegmentedControl.TabCenter>
            </SegmentedControl.Tab>
          </SegmentedControl>
          <button
            onClick={cycleMode}
...
export const AppUI_useToggle_순환_뷰_모드: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'AppUI의 useToggle([...values] as const) 패턴. const assertion으로 타입 안전한 순환 상태를 구현합니다. "순환 →" 버튼이 useToggle()의 toggle() 호출을 시뮬레이션합니다.',
      },
    },
  },
    ...
    <div style={{ maxWidth: 520, display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      >
        ...
        {Array.from({ length: pageSize }, (_, i) => {
          ...
          const statuses = [
            { label: '활성', color: '#10b981' },
            { label: '휴면', color: '#f59e0b' },
          ]
          ...
          return (
            <div
              ...
              style={{
                ...
                gridTemplateColumns: '2fr 1fr 1fr',
                padding: '10px 14px',
                borderBottom: n < to ? '1px solid var(--sem-eclipse-color-borderSubtle)' : 'none',
                fontSize: 12,
                color: 'var(--sem-eclipse-color-foregroundPrimary)',
                gap: 8,
                alignItems: 'center',
...
export const AppUI_useCounter_페이지네이션: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'AppUI의 useCounter(initial, { min, max }) 패턴. [count, { increment, decrement, set, reset }] 튜플을 반환하며, 각 핸들러가 이산적인 상태 전환을 담당합니다.',
      },
    },
  },
  ...
  return (
    <div style={{ maxWidth: 520, display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <SegmentedControl
        ...
        onTabChange={(idx) => setActiveCategory(categories[idx])}
      >
        {categories.map((cat) => (
        ...
        {FILTER_OPTIONS[activeCategory].map((opt) => {
          ...
          return (
            <button
              key={opt.id}
              onClick={() =>
                active ? remove(opt.id) : append({ ...opt, category: activeCategory })
              }
              style={{
                padding: '5px 12px',
                borderRadius: 20,
                ...
                border: `1px solid ${active ? 'var(--sem-eclipse-color-fillPrimary)' : 'var(--sem-eclipse-color-borderDefault)'}`,
                background: active
                  ? 'color-mix(in srgb, var(--sem-eclipse-color-fillPrimary) 10%, var(--sem-eclipse-color-backgroundPrimary))'
                  : 'var(--sem-eclipse-color-backgroundPrimary)',
                color: active
                  ? 'var(--sem-eclipse-color-fillPrimary)'
...
export const AppUI_useListState_필터_파이프라인: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'AppUI의 useListState([]) 패턴. [state, { append, remove, filter, ... }] 튜플로 이산적인 리스트 조작을 담당합니다. SegmentedControl로 카테고리를 전환하며 필터를 누적합니다.',
      },
    },
  },
          ...
          >
            ...
            <div
              style={{
                ...
                height: 22,
                borderRadius: '50%',
                background: avatarColor[issue.assignee],
                display: 'flex',
                alignItems: 'center',
...
function M3IssueTrackerPeriodSegmentRender() {
  ...
  const barData = data[metric].slice(
    0,
    period === 0 ? 7 : period === 1 ? 7 : period === 2 ? 6 : period === 3 ? 4 : 5
  )
  ...
  return (
    <div style={{ width: 420, display: 'flex', flexDirection: 'column', gap: 16, padding: 20 }}>
      ...
      <div
        style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 100, padding: '0 4px' }}
      >
        {barData.map((val, i) => (
          <div
            key={i}
          ...
          >
            <div
              style={{
                ...
                transition: 'height 0.3s ease',
                opacity: 0.8 + (i / barData.length) * 0.2,
              }}
            />
...
function AccessibleViewModeSegmentRender() {
  ...
  const items = [
    {
      title: 'Button 컴포넌트',
      desc: '다양한 variant와 크기를 지원하는 버튼',
      count: 24,
      tag: 'UI',
    },
    {
      title: 'TextField 입력',
      desc: '레이블 애니메이션이 있는 플로팅 텍스트 필드',
      count: 18,
      tag: 'Form',
    },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          {items.map((item) => (
            ...
            >
              ...
              <p style={{ fontSize: 10, color: 'var(--sem-eclipse-color-foregroundTertiary)' }}>
                {item.desc}
              </p>
              <p
                style={{
...
export const Accessible_콘텐츠_보기_모드_전환: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'AccessibleUI ButtonGroup 대신 SegmentedControl로 보기 모드 전환. 아이콘만 있는 Trigger로 컴팩트하게 구성. 카드/리스트/컴팩트 레이아웃을 실시간으로 전환하는 패턴.',
      },
    },
  },
  ...
  return (
    <div style={{ maxWidth: 500, fontFamily: 'system-ui, sans-serif' }}>
        ...
        <SegmentedControl selectedIndex={intervalIdx} onTabChange={setIntervalIdx}>
          {intervalKeys.map((v) => (
            <SegmentedControl.Tab key={v} value={v}>
              <SegmentedControl.TabCenter>
                <span style={{ fontSize: 11, padding: '0 2px' }}>{labels[v]}</span>
              </SegmentedControl.TabCenter>
            </SegmentedControl.Tab>
          ))}
        </SegmentedControl>
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: '3px 3px 0 0',
                  background: 'var(--sem-eclipse-color-fillPrimary)',
                  opacity: 0.7 + (idx / points.length) * 0.3,
                  transition: 'height 0.3s ease',
                }}
              />
            </div>
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        {[
          { label: '평균', val: Math.round(points.reduce((a, b) => a + b, 0) / points.length) },
          { label: '최대', val: max },
          { label: '최소', val: Math.min(...points) },
        ].map((s) => (
          <div
            key={s.label}
            style={{
...
export const AppUI_데이터_집계_단위_전환: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'AppUI SegmentedControl 패턴 — 실시간/5분/1시간/1일 집계 단위를 선택해 차트 데이터를 전환. 선택값에 따라 막대 높이와 통계값이 즉시 업데이트됩니다.',
      },
    },
  },
...
function PrimitiveEditorAlignSegmentRender() {
  ...
  const sampleText =
    '텍스트 정렬과 줄 간격은 가독성에 큰 영향을 미칩니다. Orbit UI의 SegmentedControl로 에디터 옵션을 직관적으로 제어할 수 있습니다.'
          ...
          <SegmentedControl selectedIndex={alignIdx} onTabChange={setAlignIdx}>
            {alignKeys.map((v) => {
              ...
              return (
                <SegmentedControl.Tab key={v} value={v}>
                  <SegmentedControl.TabCenter>
                    <span style={{ fontSize: 12 }}>{icons[v]}</span>
                  </SegmentedControl.TabCenter>
                </SegmentedControl.Tab>
              )
            })}
          ...
          <SegmentedControl selectedIndex={spacingIdx} onTabChange={setSpacingIdx}>
            {spacingKeys.map((v) => {
              ...
              return (
                <SegmentedControl.Tab key={v} value={v}>
                  <SegmentedControl.TabCenter>
                    <span style={{ fontSize: 11 }}>{labels[v]}</span>
                  </SegmentedControl.TabCenter>
                </SegmentedControl.Tab>
              )
            })}
...
export const Primitive_에디터_정렬_툴바: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'PrimitiveUI ToggleGroup 대신 SegmentedControl 두 개를 나란히 배치해 에디터 텍스트 정렬 + 줄 간격을 동시에 제어. 미리보기 텍스트에 실시간 반영.',
      },
    },
  },
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {filtered.length === 0 ? (
          ...
          filtered.map((iss) => (
            ...
            >
              <span style={{ fontSize: 11, fontFamily: 'monospace', color: '#94a3b8', width: 58 }}>
                {iss.id}
              </span>
              <span
                style={{
              ...
              <span
                style={{
                  ...
                  padding: '2px 7px',
                  borderRadius: 99,
                  background: `${priorityColor[iss.priority]}18`,
              ...
              <span
                style={{
                  ...
                  padding: '2px 7px',
                  borderRadius: 99,
                  background: `${statusColor[iss.status]}18`,
...
export const IssueTracker_이슈_우선순위_상태_필터: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'IssueTracker 이슈 트래커 필터 패턴. 우선순위(Urgent/High/Medium/Low) + 상태(Backlog/Todo/In Progress/Done) 2단 SegmentedControl 조합으로 이슈 교차 필터링.',
      },
    },
  },
    ...
    >
      ...
      {view === 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {metricsBase.map((m) => {
            const val =
              m.unit === '명' ? (m.value * multiplier).toLocaleString() : m.value.toFixed(1)
            return (
              <div
                key={m.label}
                style={{
...
export const UtilityCSS_대시보드_뷰_기간_이중_세그먼트: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'UtilityUI 대시보드 컨트롤 패턴. 뷰 모드(요약/상세/원시 데이터) + 기간(오늘/7일/30일/90일) 두 개의 SegmentedControl 동시 제어. 기간 배수로 메트릭 동적 계산.',
      },
    },
  },
    ...
    <div style={{ width: 520 }}>
      ...
      >
        ...
        >
          ...
          <span style={{ fontSize: 11, color: isDark ? '#64748b' : '#94a3b8', marginLeft: 4 }}>
            {langs[lang].toLowerCase()}.{['.ts', '.js', '.py', '.go'][lang].slice(1)}
          </span>
        </div>
        <pre
          style={{
...
export const IssueTracker_UtilityCSS_코드_에디터_세그먼트: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          'IssueTracker + UtilityUI 코드 에디터 패턴. 언어(TS/JS/Python/Go) + 테마(Light/Dark/System) 두 개 SegmentedControl로 코드 스니펫과 에디터 스타일 동적 전환.',
      },
    },
  },
