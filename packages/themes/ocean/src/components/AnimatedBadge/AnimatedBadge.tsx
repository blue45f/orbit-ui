import { ComponentThemeProps, Badge, findComponent, setupContext, polymorphic } from '@ui-forge/core'
...
const [AnimatedBadgeProvider, useAnimatedBadgeContext] = setupContext<{ size: AnimatedBadgeProps['size'] }>('AnimatedBadge')
    ...
    return (
      <AnimatedBadgeProvider size={size}>
        <Badge
          ...
          {...rest}
        >
          {leading && <Badge.Leading className={styles.slotLeading({ size })}>{leading}</Badge.Leading>}
...
const AnimatedBadgeLeading: React.FC<AnimatedBadgeVisualProps> = ({ children }) => {
  ...
  return <IconPropsContext value={{ size: size === 'small' ? 10 : 14 }}>{children}</IconPropsContext>
}
