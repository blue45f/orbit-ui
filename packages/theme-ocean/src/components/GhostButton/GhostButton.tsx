...
const GhostButtonRoot: React.ForwardRefExoticComponent<GhostButtonPropsWithAs & React.RefAttributes<HTMLButtonElement>> =
  forwardRef<HTMLButtonElement, GhostButtonPropsWithAs>((props, ref) => {
    ...
    return (
      ...
      >
        ...
        {trailing.length > 0 && (
          <Button.Trailing width={TEXT_BUTTON_SIZE_MAP[size].trailing.width}>{trailing}</Button.Trailing>
        )}
      </Button>
    )
  })
