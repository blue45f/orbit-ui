      ...
      <div style={{ maxWidth: '640px' }}>
        <div style={{ marginBottom: '32px' }}>
          ...
          <p style={{ fontSize: '14px', color: '#64748b', lineHeight: 1.6, margin: 0 }}>
            Orbit UI의 모든 컴포넌트는 4개의 물리적 레이어로 구성됩니다. 각 레이어는 독립적으로
            스타일과 상태를 제어합니다.
          </p>
        </div>
            ...
            <ContentLayer direction="horizontal" style={{ gap: '12px', alignItems: 'center' }}>
              ...
              <div>
                <div style={{ fontSize: '14px', fontWeight: 600, marginBottom: '2px' }}>
                  ContainerLayer + StateLayer + ContentLayer
                </div>
                <div style={{ fontSize: '12px', color: '#94a3b8' }}>
                  Hover하면 StateLayer가 반응하고, ContentLayer는 가려지지 않습니다
                </div>
              </div>
            </ContentLayer>
          </ContainerLayer>
        ...
        <div style={sectionStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px' }}>
            {[
              {
                name: 'ContainerLayer',
                desc: '최외곽 래퍼. border-radius, shadow, padding을 담당',
                color: '#64748b',
                code: '<ContainerLayer as="button">',
              },
              {
                name: 'StateLayer',
                desc: 'hover/press 시각 효과 오버레이. absolute로 컨테이너를 덮음',
                color: '#6366f1',
                code: '<StateLayer style={{ backgroundColor: "rgba(0,0,0,0.1)", transition: "background-color 0.2s ease", borderRadius: "12px" }} />',
              },
              {
                name: 'BorderLayer',
                desc: '테두리 전용 레이어. border-width/color를 독립 제어',
                color: '#3b82f6',
                code: '<BorderLayer />',
              },
              {
                name: 'ContentLayer',
                desc: '텍스트/아이콘 등 실제 콘텐츠. Flex 기반 레이아웃',
                color: '#10b981',
                code: '<ContentLayer direction="horizontal">',
              },
            ].map((l, i) => (
              <div
                key={i}
